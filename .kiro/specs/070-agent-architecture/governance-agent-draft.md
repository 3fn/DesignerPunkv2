# Product Governance Agent — Draft Sketch
...
1. **Name**: **Resolved: Stacy**, after Stacey Abrams — held democratic systems accountable to their stated principles. Ensures the product development process works as promised, gaps are identified, and nothing falls through the cracks. Parallels Thurgood (Marshall held legal systems accountable; Abrams holds democratic systems accountable).

2. **Write scope**: **Resolved.** This agent audits and documents but doesn't implement. Write scope:
   - `.kiro/specs/**` (audit findings, process docs, lessons-in-progress)
   - `docs/specs/**` (completion docs, audit reports)

3. **Relationship to Thurgood**: **Resolved.** Load Thurgood's key standards (Test-Development-Standards, Contract-System-Reference) as skill resources. Query others via MCP when specific audits require them. They share methodology but operate in different domains — Thurgood audits the system, Stacy audits the product.
...
{
  "name": "stacy",
  "description": "Product governance & quality assurance — process quality, test coverage verification, cross-platform parity auditing, spec structure governance, and lessons-learned capture",
  "prompt": "file://./stacy-prompt.md",
  "includeMcpJson": true,
  "tools": ["*"],
  "allowedTools": [
    "read",
...
- The audit vs write distinction directly mirrors Thurgood's model — proven pattern
- Three operational modes (process audit, parity review, lessons synthesis review) cover the core workflows — lessons capture is incremental within process audit, lessons synthesis is a distinct post-completion step
- The severity classification (critical/high/medium/low) matches Thurgood's audit output
- Resource list is appropriately heavy on process and standards docs (13 skill resources, trimmed Technology Stack and platform-implementation-guidelines)
- Incremental capture rule prevents session-death information loss

### Resolved since initial draft:
- Name: Stacy (Stacey Abrams) ✅
- Write scope: `.kiro/specs/**`, `docs/specs/**` ✅
- Relationship to Thurgood: Load key standards as skill resources, query others via MCP ✅
- Lessons-learned: Folded into process audit checklist with incremental capture rule ✅
- Handoff protocol: Referenced in process audit and collaboration model ✅
- Parity timing: Generalized to any single-platform start, dormant until second platform ✅

### What needs Peter's review:
- Whether the parity review mode needs more detail for when it activates

### What I'm uncertain about:
- The screen specification workflow is theoretical — actual audit patterns may look different once real product work begins
