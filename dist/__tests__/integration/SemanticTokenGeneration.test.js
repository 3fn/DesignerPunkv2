...
describe('Semantic Token Generation - End-to-End Integration', () => {
    ...
    describe('Cross-Platform Consistency', () => {
        ...
        it('should maintain identical primitive→semantic relationships across platforms', () => {
            ...
            const androidResult = generator.generateAndroidTokens();
            // Get semantic tokens
            const semanticTokens = (0, semantic_1.getAllSemanticTokens)().filter(s => !s.name.startsWith('shadow.') &&
                !s.name.startsWith('glow.'));
            ...
            semanticTokens.forEach(semantic => {
                if (semantic.primitiveReferences.value) {
                    ...
                    const webPrimitiveName = (0, PlatformNamingRules_1.getPlatformTokenName)(primitiveRef, 'web', semantic.category);
                    const iosSemanticName = (0, PlatformNamingRules_1.getPlatformTokenName)(semantic.name, 'ios', semantic.category);
                    const iosPrimitiveName = (0, PlatformNamingRules_1.getPlatformTokenName)(primitiveRef, 'ios', semantic.category);
                    const androidSemanticName = (0, PlatformNamingRules_1.getPlatformTokenName)(semantic.name, 'android', semantic.category);
                    const androidPrimitiveName = (0, PlatformNamingRules_1.getPlatformTokenName)(primitiveRef, 'android', semantic.category);
        ...
        it('should use getPlatformTokenName() for consistent cross-platform naming', () => {
            ...
            const webName = (0, PlatformNamingRules_1.getPlatformTokenName)(testTokenName, 'web', 'color');
            expect(webName).toBe('--color-primary');
            // iOS: camelCase
            const iosName = (0, PlatformNamingRules_1.getPlatformTokenName)(testTokenName, 'ios', 'color');
            expect(iosName).toBe('colorPrimary');
            // Android: snake_case
            const androidName = (0, PlatformNamingRules_1.getPlatformTokenName)(testTokenName, 'android', 'color');
    ...
    describe('Backward Compatibility', () => {
        it('should maintain primitive token output unchanged', () => {
            ...
            primitives.forEach(token => {
                const webName = (0, PlatformNamingRules_1.getPlatformTokenName)(token.name, 'web', token.category);
                expect(webResult.content).toContain(webName);
                const iosName = (0, PlatformNamingRules_1.getPlatformTokenName)(token.name, 'ios', token.category);
                expect(iosResult.content).toContain(iosName);
                const androidName = (0, PlatformNamingRules_1.getPlatformTokenName)(token.name, 'android', token.category);
