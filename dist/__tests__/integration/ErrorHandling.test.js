...
const BuildErrorHandler_1 = require("../../integration/BuildErrorHandler");
const BuildSystemInterface_1 = require("../../integration/BuildSystemInterface");
describe('Error Handling Integration', () => {
    let errorHandler;
    beforeEach(() => {
        errorHandler = new BuildErrorHandler_1.BuildErrorHandler({
    ...
    describe('Error Creation and Handling', () => {
        it('should create configuration error with context', () => {
            ...
            const error = BuildErrorHandler_1.BuildErrorHandler.configurationError('Invalid configuration', config, ['Check targets array', 'Ensure at least one platform']);
        ...
        it('should create platform error with suggestions', () => {
            const error = BuildErrorHandler_1.BuildErrorHandler.platformError('web', 'Platform generation failed', ['Verify platform configuration', 'Check dependencies']);
        ...
        it('should create generation error', () => {
            const error = BuildErrorHandler_1.BuildErrorHandler.generationError('Token generation failed', 50, ['Check token definitions', 'Verify dependencies']);
        ...
        it('should create validation error', () => {
            ...
            const error = BuildErrorHandler_1.BuildErrorHandler.validationError('Validation failed', validationErrors, ['Fix mathematical relationships', 'Check token references']);
            expect(error.code).toBe('VALIDATION_ERROR');
            expect(error.category).toBe('validation');
            expect(error.context).toHaveProperty('validationErrors');
        });
        it('should create warning', () => {
            const warning = BuildErrorHandler_1.BuildErrorHandler.warning('Performance degradation detected', 'integration', ['Optimize token generation', 'Enable caching']);
    ...
    describe('Error Recovery', () => {
        it('should recover with default platform', async () => {
            const error = BuildErrorHandler_1.BuildErrorHandler.platformError('ios', 'Platform unavailable');
        ...
        it('should recover by skipping platform', async () => {
            const skipHandler = new BuildErrorHandler_1.BuildErrorHandler({
                strategy: 'skip-platform',
                defaultPlatform: 'web'
            });
            const error = BuildErrorHandler_1.BuildErrorHandler.platformError('android', 'Platform error');
        ...
        it('should recover from cache', async () => {
            const cacheHandler = new BuildErrorHandler_1.BuildErrorHandler({
                strategy: 'use-cache',
                enableCache: true
            });
            const error = BuildErrorHandler_1.BuildErrorHandler.generationError('Generation failed');
        ...
        it('should require manual intervention', async () => {
            const manualHandler = new BuildErrorHandler_1.BuildErrorHandler({
                strategy: 'manual'
            });
            const error = BuildErrorHandler_1.BuildErrorHandler.configurationError('Critical error');
        ...
        it('should abort on critical errors', async () => {
            const abortHandler = new BuildErrorHandler_1.BuildErrorHandler({
                strategy: 'abort'
            });
            const error = BuildErrorHandler_1.BuildErrorHandler.configurationError('Critical configuration error');
        ...
        it('should use custom error handler', async () => {
            ...
            const customHandler = new BuildErrorHandler_1.BuildErrorHandler({
                strategy: 'use-default',
                customHandler: async (error) => {
                    customHandlerCalled = true;
                }
            });
            const error = BuildErrorHandler_1.BuildErrorHandler.generationError('Test error');
    ...
    describe('Error Filtering and Querying', () => {
        beforeEach(() => {
            // Add various errors
            errorHandler.handleError(BuildErrorHandler_1.BuildErrorHandler.configurationError('Config error 1'));
            errorHandler.handleError(BuildErrorHandler_1.BuildErrorHandler.platformError('web', 'Platform error 1'));
            errorHandler.handleError(BuildErrorHandler_1.BuildErrorHandler.warning('Warning 1', 'integration'));
            errorHandler.handleError(BuildErrorHandler_1.BuildErrorHandler.generationError('Generation error 1'));
        });
    ...
    describe('Error Formatting', () => {
        it('should format error with all details', () => {
            const error = BuildErrorHandler_1.BuildErrorHandler.configurationError('Invalid build configuration', { system: 'webpack', targets: [] }, ['Check configuration', 'Add targets']);
    ...
    describe('Build System Integration Error Scenarios', () => {
        ...
        beforeEach(() => {
            buildIntegration = new BuildSystemInterface_1.BuildSystemIntegration();
        });
    ...
    describe('Fallback Strategy Effectiveness', () => {
        it('should successfully use default fallback', async () => {
            const error = BuildErrorHandler_1.BuildErrorHandler.platformError('ios', 'Unavailable');
            const result = await errorHandler.recover(error);
            expect(result.recovered).toBe(true);
            expect(result.actions.length).toBeGreaterThan(0);
        });
        it('should handle fallback failure gracefully', async () => {
            const error = BuildErrorHandler_1.BuildErrorHandler.generationError('Critical failure');
            const result = await errorHandler.recover(error);
            // Should still attempt recovery
            expect(result.strategy).toBe('use-default');
        });
        it('should track recovery actions', async () => {
            const error = BuildErrorHandler_1.BuildErrorHandler.platformError('android', 'Error');
            const result = await errorHandler.recover(error);
            expect(result.actions).toBeInstanceOf(Array);
            expect(result.actions.length).toBeGreaterThan(0);
        });
        it('should generate warnings during recovery', async () => {
            const error = BuildErrorHandler_1.BuildErrorHandler.platformError('ios', 'Platform error');
            const result = await errorHandler.recover(error);
    ...
    describe('Error Context Preservation', () => {
        ...
        it('should handle BuildError directly', () => {
            const buildError = BuildErrorHandler_1.BuildErrorHandler.configurationError('Config error');
    ...
    describe('Error Recovery Limits', () => {
        it('should respect max retries configuration', () => {
            const handler = new BuildErrorHandler_1.BuildErrorHandler({
                strategy: 'use-default',
                maxRetries: 3
            });
        ...
        it('should handle cache unavailability', async () => {
            const noCacheHandler = new BuildErrorHandler_1.BuildErrorHandler({
                strategy: 'use-cache',
                enableCache: false
            });
            const error = BuildErrorHandler_1.BuildErrorHandler.generationError('Generation failed');
    ...
    describe('Error Message Clarity', () => {
        it('should provide clear configuration error messages', () => {
            const error = BuildErrorHandler_1.BuildErrorHandler.configurationError('Missing required field: targets', undefined, ['Add targets array to configuration', 'Specify at least one platform']);
            expect(error.message).toContain('Missing required field');
            expect(error.suggestions).toHaveLength(2);
            expect(error.suggestions[0]).toContain('Add targets array');
        });
        it('should provide clear platform error messages', () => {
            const error = BuildErrorHandler_1.BuildErrorHandler.platformError('ios', 'Swift compiler not found', ['Install Xcode', 'Configure Swift toolchain']);
        ...
        it('should provide actionable suggestions', () => {
            const error = BuildErrorHandler_1.BuildErrorHandler.validationError('Token validation failed', ['space100: baseline grid violation', 'color.primary: invalid reference'], ['Fix baseline grid alignment', 'Update color reference']);
            expect(error.suggestions).toHaveLength(2);
