...
const BlendValueGenerator_1 = require("../../generators/BlendValueGenerator");
const BlendUtilityGenerator_1 = require("../../generators/BlendUtilityGenerator");
const BlendTokens_1 = require("../../tokens/BlendTokens");
const BlendCalculator_1 = require("../../blend/BlendCalculator");
const BlendTokens_2 = require("../../tokens/BlendTokens");
describe('Blend Cross-Platform Consistency', () => {
    let valueGenerator;
    let utilityGenerator;
    let calculator;
    beforeEach(() => {
        valueGenerator = new BlendValueGenerator_1.BlendValueGenerator();
        utilityGenerator = new BlendUtilityGenerator_1.BlendUtilityGenerator();
    ...
    describe('Blend Value Consistency', () => {
        ...
        test('base value is consistent across platforms', () => {
            ...
            const iosResult = valueGenerator.generateiOSBlendValues({ includeComments: false });
            const androidResult = valueGenerator.generateAndroidBlendValues({ includeComments: false });
            expect(webResult).toContain(`BLEND_BASE_VALUE = ${BlendTokens_1.BLEND_BASE_VALUE}`);
            expect(iosResult).toContain(`baseValue: Double = ${BlendTokens_1.BLEND_BASE_VALUE}`);
            expect(androidResult).toContain(`baseValue = ${BlendTokens_1.BLEND_BASE_VALUE}f`);
        });
    ...
    describe('Specific Color Blend Consistency', () => {
        test('purple500 with blend200 darker produces same result on all platforms', () => {
            ...
            const result = calculator.calculateBlend('#A855F7', { baseValue: 0.08 }, BlendTokens_2.BlendDirection.DARKER);
        ...
        test('purple500 with blend200 lighter produces same result on all platforms', () => {
            const result = calculator.calculateBlend('#A855F7', { baseValue: 0.08 }, BlendTokens_2.BlendDirection.LIGHTER);
            expect(result).toMatch(/^#[0-9A-F]{6}$/);
        });
        test('purple500 with blend200 saturate produces same result on all platforms', () => {
            const result = calculator.calculateBlend('#A855F7', { baseValue: 0.08 }, BlendTokens_2.BlendDirection.SATURATE);
            expect(result).toMatch(/^#[0-9A-F]{6}$/);
        });
        test('purple500 with blend200 desaturate produces same result on all platforms', () => {
            const result = calculator.calculateBlend('#A855F7', { baseValue: 0.08 }, BlendTokens_2.BlendDirection.DESATURATE);
            expect(result).toMatch(/^#[0-9A-F]{6}$/);
        });
    });
    describe('All Blend Directions Consistency', () => {
        test('darker blend is consistent across all platforms', () => {
            ...
            testColors.forEach(color => {
                testBlendValues.forEach(blendValue => {
                    const result = calculator.calculateBlend(color, { baseValue: blendValue }, BlendTokens_2.BlendDirection.DARKER);
        ...
        test('lighter blend is consistent across all platforms', () => {
            ...
            testColors.forEach(color => {
                testBlendValues.forEach(blendValue => {
                    const result = calculator.calculateBlend(color, { baseValue: blendValue }, BlendTokens_2.BlendDirection.LIGHTER);
                    expect(result).toMatch(/^#[0-9A-F]{6}$/);
                });
            });
        });
        test('saturate blend is consistent across all platforms', () => {
            ...
            testColors.forEach(color => {
                testBlendValues.forEach(blendValue => {
                    const result = calculator.calculateBlend(color, { baseValue: blendValue }, BlendTokens_2.BlendDirection.SATURATE);
                    expect(result).toMatch(/^#[0-9A-F]{6}$/);
                });
            });
        });
        test('desaturate blend is consistent across all platforms', () => {
            ...
            testColors.forEach(color => {
                testBlendValues.forEach(blendValue => {
                    const result = calculator.calculateBlend(color, { baseValue: blendValue }, BlendTokens_2.BlendDirection.DESATURATE);
                    expect(result).toMatch(/^#[0-9A-F]{6}$/);
                });
            });
        });
...
//# sourceMappingURL=BlendCrossPlatformConsistency.test.js.map