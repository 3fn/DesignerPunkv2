...
class SafetyValidator {
     ...
     */
    async validateReleaseSafety(releasePlan) {
        const errors = [];
        const warnings = [];
     ...
     */
    async validateDependencyConflicts(releasePlan) {
        const errors = [];
        const warnings = [];
     ...
     */
    async validatePublishingSafety(releasePlan) {
        const errors = [];
        const warnings = [];
     ...
     * Validate major release confirmation
     */
    validateMajorReleaseConfirmation(releasePlan) {
        const errors = [];
        const warnings = [];
     ...
     * Validate breaking change confirmation
     */
    validateBreakingChangeConfirmation(releasePlan) {
        const errors = [];
        const warnings = [];
        if (releasePlan.releaseNotes.breakingChanges.length > 0) {
     ...
     * Validate rollback plan content
     */
    validateRollbackPlanContent(content, planPath) {
        const errors = [];
        const warnings = [];
     ...
     */
    async validateGitStateForRollback() {
        const errors = [];
        const warnings = [];
        try {
     ...
     * Validate backup capabilities
     */
    async validateBackupCapabilities(releasePlan) {
        const errors = [];
        const warnings = [];
     ...
     */
    checkVersionConflict(depUpdate, packages) {
     ...
     * Check for missing dependencies
     */
    async checkMissingDependencies(packages) {
        const missingDeps = [];
        for (const pkg of packages) {
     ...
     * Validate publishing order
     */
    validatePublishingOrder(publishingPlan) {
        const errors = [];
        const warnings = [];
     ...
     */
    async validateRegistryAccessibility(packages) {
        const errors = [];
        const warnings = [];
     ...
     * Validate build artifacts exist
     */
    async validateBuildArtifacts(packages) {
        const errors = [];
        const warnings = [];
        for (const pkg of packages) {
     ...
     */
    async checkDependencyExists(depName) {
