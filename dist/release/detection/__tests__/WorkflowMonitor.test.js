...
describe('WorkflowMonitor', () => {
    ...
    beforeEach(() => {
        config = {
            ...ReleaseConfig_1.DEFAULT_RELEASE_CONFIG.detection,
            monitorPaths: [
    ...
    describe('Event Detection', () => {
        it('should detect task completion events', async () => {
            ...
            mockFs.readdir.mockResolvedValue(completionFiles);
    ...
    describe('Event Queue Management', () => {
        it('should queue events and process them in order', async () => {
            ...
            monitor.on('event-processed', (event) => {
        ...
        it('should respect queue size limits', async () => {
            const smallQueueMonitor = new WorkflowMonitor_1.WorkflowMonitor(config, {
                pollInterval: 100,
                enableFileWatching: false,
                enableHookIntegration: false
            });
            // Set small queue size for testing
            smallQueueMonitor.eventQueue.maxSize = 2;
    ...
    describe('Hook Integration', () => {
        it('should detect git commit events for task completion', async () => {
            ...
            monitor.on('workflow-event', (event) => {
                detectedEvents.push(event);
            });
            // Simulate git commit monitoring
            await monitor.setupGitCommitMonitoring();
            // Advance timers to trigger git check
            jest.advanceTimersByTime(10000);
            await new Promise(resolve => setTimeout(resolve, 0));
            expect(detectedEvents.some(e => e.type === 'task-completion' &&
                e.metadata.taskNumber === '1.1' &&
                e.metadata.commitHash === commitHash)).toBe(true);
        });
        it('should process trigger files from hook system', async () => {
            ...
            mockFs.readdir.mockResolvedValue(['trigger-1.json']);
            ...
            await monitor.setupHookIntegration();
            // Advance timers to trigger check
            jest.advanceTimersByTime(2000);
            await new Promise(resolve => setTimeout(resolve, 0));
            expect(detectedEvents.some(e => e.type === 'task-completion' &&
                e.metadata.taskName === '1.1 Test task')).toBe(true);
        });
...
`;
            ...
            await monitor.setupFileOrganizationMonitoring();
            // Advance timers to trigger check
            jest.advanceTimersByTime(5000);
            await new Promise(resolve => setTimeout(resolve, 0));
            expect(detectedEvents.some(e => e.type === 'file-change' &&
                e.metadata.logEntry?.includes('completion.md'))).toBe(true);
        });
    });
...
`;
        ...
        it('should process spec completion events', async () => {
            ...
            const emittedEvents = [];
            monitor.on('spec-completion-detected', (data) => {
                emittedEvents.push(data);
            });
            await monitor.processWorkflowEvent(event);
            expect(emittedEvents).toHaveLength(1);
...
`;
            mockFs.readFile.mockResolvedValue(tasksContent);
            const emittedEvents = [];
            monitor.on('task-status-changed', (data) => {
                emittedEvents.push(data);
            });
            await monitor.processWorkflowEvent(event);
    ...
    describe('Monitoring Lifecycle', () => {
        ...
        it('should not start monitoring if already monitoring', async () => {
            ...
            const secondState = monitor.isMonitoring;
    ...
    describe('Path Expansion and Matching', () => {
        it('should expand glob paths correctly', async () => {
            ...
            const expandedPaths = await monitor.expandGlobPath(globPath);
        ...
        it('should match glob patterns correctly', async () => {
            const matchesPattern = monitor.matchesPattern.bind(monitor);
        ...
        it('should determine event types from file paths', async () => {
            const determineEventType = monitor.determineEventType.bind(monitor);
    ...
    describe('Error Handling', () => {
        ...
        it('should handle malformed trigger files', async () => {
            mockFs.readdir.mockResolvedValue(['invalid-trigger.json']);
...
`;
            const extractTaskName = monitor.extractTaskName.bind(monitor);
...
//# sourceMappingURL=WorkflowMonitor.test.js.map