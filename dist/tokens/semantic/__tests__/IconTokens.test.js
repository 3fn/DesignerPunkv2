...
const IconTokens_1 = require("../IconTokens");
...
describe('Icon Size Token Calculation', () => {
    describe('calculateIconSize()', () => {
        it('should calculate icon size from fontSize × lineHeight', () => {
            const fontSize = FontSizeTokens_1.fontSizeTokens.fontSize100; // 16
            const lineHeight = LineHeightTokens_1.lineHeightTokens.lineHeight100; // 1.5
            const iconSize = (0, IconTokens_1.calculateIconSize)(fontSize, lineHeight);
            expect(iconSize).toBe(24); // 16 × 1.5 = 24
        });
        it('should round non-integer results', () => {
            const fontSize = FontSizeTokens_1.fontSizeTokens.fontSize075; // 14
            const lineHeight = LineHeightTokens_1.lineHeightTokens.lineHeight075; // 1.25
            const iconSize = (0, IconTokens_1.calculateIconSize)(fontSize, lineHeight);
        ...
        it('should calculate all expected icon sizes correctly', () => {
            ...
            Object.entries(expectedSizes).forEach(([scale, expectedSize]) => {
                const fontSize = FontSizeTokens_1.fontSizeTokens[`fontSize${scale}`];
                const lineHeight = LineHeightTokens_1.lineHeightTokens[`lineHeight${scale}`];
                const calculatedSize = (0, IconTokens_1.calculateIconSize)(fontSize, lineHeight);
    ...
    describe('generateIconSizeTokens()', () => {
        it('should generate all 11 icon size tokens', () => {
            const tokens = (0, IconTokens_1.generateIconSizeTokens)();
        ...
        it('should generate tokens with correct structure', () => {
            ...
            expect(token.category).toBe(SemanticToken_1.SemanticCategory.ICON);
        });
    ...
    describe('iconTokens', () => {
        it('should have all 11 icon size tokens', () => {
            expect(Object.keys(IconTokens_1.iconTokens)).toHaveLength(11);
        });
        it('should have correct token structure', () => {
            Object.values(IconTokens_1.iconTokens).forEach(token => {
                expect(token).toHaveProperty('name');
                expect(token).toHaveProperty('primitiveReferences');
                expect(token).toHaveProperty('category');
                expect(token).toHaveProperty('context');
        ...
        it('should reference valid primitive tokens', () => {
            Object.values(IconTokens_1.iconTokens).forEach(token => {
                ...
                expect(FontSizeTokens_1.fontSizeTokens).toHaveProperty(fontSizeName);
                expect(LineHeightTokens_1.lineHeightTokens).toHaveProperty(lineHeightName);
            });
        });
    });
    describe('getAllIconTokens()', () => {
        it('should return all icon tokens as array', () => {
            const tokens = (0, IconTokens_1.getAllIconTokens)();
    ...
    describe('Token Convergence', () => {
        it('should have unique icon size values', () => {
            ...
            Object.entries(IconTokens_1.iconTokens).forEach(([name, token]) => {
                const scale = name.replace('icon.size', '');
                const fontSize = FontSizeTokens_1.fontSizeTokens[`fontSize${scale}`];
                const lineHeight = LineHeightTokens_1.lineHeightTokens[`lineHeight${scale}`];
                const size = (0, IconTokens_1.calculateIconSize)(fontSize, lineHeight);
        ...
        it('should have natural convergence at 32px', () => {
            ...
            const size125 = (0, IconTokens_1.calculateIconSize)(FontSizeTokens_1.fontSizeTokens.fontSize125, LineHeightTokens_1.lineHeightTokens.lineHeight125);
            const size200 = (0, IconTokens_1.calculateIconSize)(FontSizeTokens_1.fontSizeTokens.fontSize200, LineHeightTokens_1.lineHeightTokens.lineHeight200);
            const size300 = (0, IconTokens_1.calculateIconSize)(FontSizeTokens_1.fontSizeTokens.fontSize300, LineHeightTokens_1.lineHeightTokens.lineHeight300);
    ...
    describe('4pt Subgrid Alignment', () => {
        ...
        it('should document non-aligned sizes as acceptable trade-offs', () => {
            ...
            const allSizes = Object.entries(IconTokens_1.iconTokens).map(([name, token]) => {
                const scale = name.replace('icon.size', '');
                const fontSize = FontSizeTokens_1.fontSizeTokens[`fontSize${scale}`];
                const lineHeight = LineHeightTokens_1.lineHeightTokens[`lineHeight${scale}`];
                return (0, IconTokens_1.calculateIconSize)(fontSize, lineHeight);
            });
...
describe('Icon Size Token Structure Validation', () => {
    ...
    describe('Primitive References Validation', () => {
        ...
        it('should reference valid primitive tokens', () => {
            Object.values(IconTokens_1.iconTokens).forEach(token => {
                ...
                const fontSize = FontSizeTokens_1.fontSizeTokens[fontSizeName];
                const lineHeight = LineHeightTokens_1.lineHeightTokens[lineHeightName];
    ...
    describe('SemanticCategory.ICON Validation', () => {
        ...
        it('should not use other semantic categories', () => {
            const otherCategories = [
                ...
                SemanticToken_1.SemanticCategory.LAYERING,
    ...
    describe('Context Field Validation', () => {
        ...
        it('should reference typography styles in context', () => {
            ...
            expect(IconTokens_1.iconTokens['icon.size050'].context).toContain('caption');
            expect(IconTokens_1.iconTokens['icon.size075'].context).toContain('bodySm');
            expect(IconTokens_1.iconTokens['icon.size100'].context).toContain('bodyMd');
            expect(IconTokens_1.iconTokens['icon.size125'].context).toContain('bodyLg');
            expect(IconTokens_1.iconTokens['icon.size150'].context).toContain('h6');
            expect(IconTokens_1.iconTokens['icon.size200'].context).toContain('h5');
            expect(IconTokens_1.iconTokens['icon.size300'].context).toContain('h4');
            expect(IconTokens_1.iconTokens['icon.size400'].context).toContain('h3');
            expect(IconTokens_1.iconTokens['icon.size500'].context).toContain('h2');
            expect(IconTokens_1.iconTokens['icon.size600'].context).toContain('h1');
            expect(IconTokens_1.iconTokens['icon.size700'].context).toContain('display');
        });
        it('should provide usage guidance in context', () => {
            Object.values(IconTokens_1.iconTokens).forEach(token => {
                // Context should help developers understand when to use this size
                const hasTypographyReference = token.context.includes('body') ||
    ...
    describe('Description Field Validation', () => {
        ...
        it('should indicate rounding when applicable', () => {
            ...
            tokensWithRounding.forEach(tokenName => {
                const token = IconTokens_1.iconTokens[tokenName];
    ...
    describe('Token Consistency', () => {
        it('should have consistent structure across all tokens', () => {
            const firstToken = IconTokens_1.iconTokens['icon.size050'];
        ...
        it('should maintain name consistency with object key', () => {
            Object.entries(IconTokens_1.iconTokens).forEach(([key, token]) => {
    ...
    describe('Generated Tokens Validation', () => {
        it('should match manually defined tokens', () => {
            const generatedTokens = (0, IconTokens_1.generateIconSizeTokens)();
            // Verify same number of tokens
            expect(Object.keys(generatedTokens)).toHaveLength(Object.keys(IconTokens_1.iconTokens).length);
            // Verify each generated token matches manual definition
            Object.entries(IconTokens_1.iconTokens).forEach(([name, manualToken]) => {
...
//# sourceMappingURL=IconTokens.test.js.map