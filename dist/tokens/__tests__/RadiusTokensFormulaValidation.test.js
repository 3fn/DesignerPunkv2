...
describe('RadiusTokens Formula Validation', () => {
    ...
    describe('Formula-based tokens match original hard values', () => {
        it('radius000: RADIUS_BASE_VALUE * 0 should equal 0', () => {
            const expected = 0;
            const calculated = RadiusTokens_1.RADIUS_BASE_VALUE * 0;
            expect(calculated).toBe(expected);
            expect(RadiusTokens_1.radiusTokens.radius000.baseValue).toBe(expected);
        });
        it('radius025: RADIUS_BASE_VALUE * 0.25 should equal 2', () => {
            const expected = 2;
            const calculated = RadiusTokens_1.RADIUS_BASE_VALUE * 0.25;
            expect(calculated).toBe(expected);
            expect(RadiusTokens_1.radiusTokens.radius025.baseValue).toBe(expected);
        });
        it('radius050: RADIUS_BASE_VALUE * 0.5 should equal 4', () => {
            const expected = 4;
            const calculated = RadiusTokens_1.RADIUS_BASE_VALUE * 0.5;
            expect(calculated).toBe(expected);
            expect(RadiusTokens_1.radiusTokens.radius050.baseValue).toBe(expected);
        });
        it('radius100: RADIUS_BASE_VALUE should equal 8', () => {
            const expected = 8;
            const calculated = RadiusTokens_1.RADIUS_BASE_VALUE;
            expect(calculated).toBe(expected);
            expect(RadiusTokens_1.radiusTokens.radius100.baseValue).toBe(expected);
        });
        it('radius150: RADIUS_BASE_VALUE * 1.5 should equal 12', () => {
            const expected = 12;
            const calculated = RadiusTokens_1.RADIUS_BASE_VALUE * 1.5;
            expect(calculated).toBe(expected);
            expect(RadiusTokens_1.radiusTokens.radius150.baseValue).toBe(expected);
        });
        it('radius200: RADIUS_BASE_VALUE * 2 should equal 16', () => {
            const expected = 16;
            const calculated = RadiusTokens_1.RADIUS_BASE_VALUE * 2;
            expect(calculated).toBe(expected);
            expect(RadiusTokens_1.radiusTokens.radius200.baseValue).toBe(expected);
        });
        it('radius300: RADIUS_BASE_VALUE * 3 should equal 24', () => {
            const expected = 24;
            const calculated = RadiusTokens_1.RADIUS_BASE_VALUE * 3;
            expect(calculated).toBe(expected);
            expect(RadiusTokens_1.radiusTokens.radius300.baseValue).toBe(expected);
        });
        it('radius400: RADIUS_BASE_VALUE * 4 should equal 32', () => {
            const expected = 32;
            const calculated = RadiusTokens_1.RADIUS_BASE_VALUE * 4;
            expect(calculated).toBe(expected);
            expect(RadiusTokens_1.radiusTokens.radius400.baseValue).toBe(expected);
        });
    });
    describe('Strategic flexibility tokens preserve original values', () => {
        it('radius075: should preserve hard value of 6 (not formula-based)', () => {
            const expected = 6;
            expect(RadiusTokens_1.radiusTokens.radius075.baseValue).toBe(expected);
        ...
        it('radius125: should preserve hard value of 10 (not formula-based)', () => {
            const expected = 10;
            expect(RadiusTokens_1.radiusTokens.radius125.baseValue).toBe(expected);
        ...
        it('radius250: should preserve hard value of 20 (not formula-based)', () => {
            const expected = 20;
            expect(RadiusTokens_1.radiusTokens.radius250.baseValue).toBe(expected);
        ...
        it('radiusFull: should preserve hard value of 9999 (special case)', () => {
            const expected = 9999;
            expect(RadiusTokens_1.radiusTokens.radiusFull.baseValue).toBe(expected);
    ...
    describe('Platform values match baseValue', () => {
        it('should generate correct platform values for formula-based tokens', () => {
            ...
            testCases.forEach(({ token, expected }) => {
                const radiusToken = RadiusTokens_1.radiusTokens[token];
    ...
    describe('Complete validation: 100% match rate', () => {
        it('should verify all 12 radius tokens have correct values', () => {
            ...
            Object.entries(expectedValues).forEach(([tokenName, expectedValue]) => {
                totalCount++;
                const actualValue = RadiusTokens_1.radiusTokens[tokenName].baseValue;
        ...
        it('should verify all formula-based tokens calculate correctly', () => {
            const formulaTokens = [
                { name: 'radius000', formula: () => RadiusTokens_1.RADIUS_BASE_VALUE * 0, expected: 0 },
                { name: 'radius025', formula: () => RadiusTokens_1.RADIUS_BASE_VALUE * 0.25, expected: 2 },
                { name: 'radius050', formula: () => RadiusTokens_1.RADIUS_BASE_VALUE * 0.5, expected: 4 },
                { name: 'radius100', formula: () => RadiusTokens_1.RADIUS_BASE_VALUE, expected: 8 },
                { name: 'radius150', formula: () => RadiusTokens_1.RADIUS_BASE_VALUE * 1.5, expected: 12 },
                { name: 'radius200', formula: () => RadiusTokens_1.RADIUS_BASE_VALUE * 2, expected: 16 },
                { name: 'radius300', formula: () => RadiusTokens_1.RADIUS_BASE_VALUE * 3, expected: 24 },
                { name: 'radius400', formula: () => RadiusTokens_1.RADIUS_BASE_VALUE * 4, expected: 32 }
            ];
            formulaTokens.forEach(({ name, formula, expected }) => {
                const calculated = formula();
                const actual = RadiusTokens_1.radiusTokens[name].baseValue;
                expect(calculated).toBe(expected);
...
//# sourceMappingURL=RadiusTokensFormulaValidation.test.js.map