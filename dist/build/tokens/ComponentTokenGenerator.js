...
exports.ComponentTokenGenerator = void 0;
...
class ComponentTokenGenerator {
    ...
    validate(token) {
        ...
        if (!isBaselineAligned && token.category === 'spacing') {
            warnings.push(`Base value ${token.baseValue} does not align with ${baselineGrid}-unit baseline grid. ` +
        ...
        if (similarPrimitive) {
            warnings.push(`Similar primitive token '${similarPrimitive.name}' exists with baseValue ${similarPrimitive.baseValue}. ` +
        ...
        if (token.usage.usageCount > 10 && token.usage.appropriateUsageRate >= 80) {
            suggestions.push(`Token has high usage (${token.usage.usageCount} times) with ${token.usage.appropriateUsageRate}% appropriate usage. ` +
                `Consider promoting to primitive token.`);
        }
        const mathematicallyConsistent = errors.length === 0 &&
     ...
     */
    checkPromotion(token) {
        const usageThreshold = 10;
        const appropriateUsageThreshold = 80;
        const shouldPromote = token.usage.usageCount >= usageThreshold &&
     ...
     */
    generatePlatformValues(baseValue, tokenName) {
        return {
            ios: {
                value: baseValue,
                unit: 'pt',
     ...
     */
    validatePlatformConsistency(token) {
        const errors = [];
     ...
     */
    findSimilarPrimitiveToken(token) {
...
exports.ComponentTokenGenerator = ComponentTokenGenerator;
//# sourceMappingURL=ComponentTokenGenerator.js.map