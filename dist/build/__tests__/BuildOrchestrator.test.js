...
const BuildOrchestrator_1 = require("../BuildOrchestrator");
const BuildConfig_1 = require("../types/BuildConfig");
describe('BuildOrchestrator', () => {
    let orchestrator;
    beforeEach(() => {
        orchestrator = new BuildOrchestrator_1.BuildOrchestrator();
    });
    describe('Configuration Validation', () => {
        it('should validate valid configuration', () => {
            const config = {
                ...BuildConfig_1.DEFAULT_BUILD_CONFIG,
                platforms: ['web'],
                outputDir: './dist',
            };
        ...
        it('should reject Android configuration with invalid SDK versions', () => {
            ...
            const result = orchestrator.validateConfig(config);
            expect(result.valid).toBe(false);
            expect(result.errors.some(e => e.includes('minSdkVersion cannot be greater than targetSdkVersion'))).toBe(true);
        });
        ...
        it('should warn about source maps in production mode', () => {
            ...
            const result = orchestrator.validateConfig(config);
            expect(result.valid).toBe(true);
            expect(result.warnings.some(w => w.includes('Source maps enabled in production mode'))).toBe(true);
        });
        it('should warn about disabled minification in production mode', () => {
            ...
            const result = orchestrator.validateConfig(config);
            expect(result.valid).toBe(true);
            expect(result.warnings.some(w => w.includes('Minification disabled in production mode'))).toBe(true);
        });
    });
    ...
    describe('Platform Selection', () => {
        it('should reject invalid platforms', async () => {
            ...
            orchestrator.configure(config);
            await expect(orchestrator.build(['invalid'])).rejects.toThrow('Invalid platforms specified');
        });
        it('should reject empty platform list', async () => {
            ...
            orchestrator.configure(config);
            await expect(orchestrator.build([])).rejects.toThrow('No platforms specified for build');
        });
...
//# sourceMappingURL=BuildOrchestrator.test.js.map