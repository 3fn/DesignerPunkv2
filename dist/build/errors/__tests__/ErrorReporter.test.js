...
describe('ErrorReporter', () => {
    describe('generateReport', () => {
        it('should generate report with no errors', () => {
            const reporter = new ErrorReporter_1.ErrorReporter();
    ...
    describe('generateErrorContext', () => {
        ...
        it('should include context details when enabled', () => {
            const reporter = new ErrorReporter_1.ErrorReporter({ includeContext: true });
            const error = (0, BuildError_1.createBuildError)({
                code: 'TEST_ERROR',
                message: 'Test error',
                severity: 'error',
        ...
        it('should include stack trace when enabled', () => {
            const reporter = new ErrorReporter_1.ErrorReporter({ includeStackTraces: true });
    ...
    describe('output formats', () => {
        const testError = (0, BuildError_1.createBuildError)({
            code: 'TEST_ERROR',
            message: 'Test error',
            severity: 'error',
            category: 'build',
        ...
        it('should format report as text', () => {
            const reporter = new ErrorReporter_1.ErrorReporter({ format: 'text' });
        ...
        it('should format report as markdown', () => {
            const reporter = new ErrorReporter_1.ErrorReporter({ format: 'markdown' });
            const report = reporter.generateReport([testError]);
            expect(report.formattedReport).toContain('# Build Error Report');
            expect(report.formattedReport).toContain('## Summary');
            expect(report.formattedReport).toContain('## Critical Errors');
        ...
        it('should format report as JSON', () => {
            const reporter = new ErrorReporter_1.ErrorReporter({ format: 'json' });
            const report = reporter.generateReport([testError]);
    ...
    describe('report options', () => {
        it('should limit detailed errors based on maxDetailedErrors', () => {
            const reporter = new ErrorReporter_1.ErrorReporter({ maxDetailedErrors: 2 });
            const errors = [
                (0, BuildError_1.createBuildError)({
                    code: 'ERROR_1',
                    message: 'Error 1',
        ...
        it('should exclude recommendations when disabled', () => {
            const reporter = new ErrorReporter_1.ErrorReporter({ includeRecommendations: false });
            const errors = [
                (0, BuildError_1.createBuildError)({
                    code: 'ERROR_1',
                    message: 'Error 1',
