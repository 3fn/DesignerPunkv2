...
const iOSBuilder_1 = require("../iOSBuilder");
...
describe('iOSBuilder', () => {
    ...
    let testOutputDir;
    beforeEach(() => {
        builder = new iOSBuilder_1.iOSBuilder();
    ...
    describe('constructor', () => {
        ...
        it('should accept custom iOS build options', () => {
            const customBuilder = new iOSBuilder_1.iOSBuilder({
                swiftVersion: '5.10',
...
`;
    ...
    describe('Swift Package structure', () => {
        it('should generate Package.swift with proper dependencies', async () => {
            ...
            const packageManifest = await fs.readFile(path.join(testOutputDir, 'ios', 'Package.swift'), 'utf-8');
        ...
        it('should create proper Swift source file organization', async () => {
            ...
            const spacingTokensFile = await fs.readFile(path.join(sourcesDir, 'Tokens', 'SpacingTokens.swift'), 'utf-8');
            expect(spacingTokensFile).toContain('enum Space');
            const colorTokensFile = await fs.readFile(path.join(sourcesDir, 'Tokens', 'ColorTokens.swift'), 'utf-8');
            ...
            const colorExtensions = await fs.readFile(path.join(sourcesDir, 'Extensions', 'ColorExtensions.swift'), 'utf-8');
            expect(colorExtensions).toContain('extension Color');
            expect(colorExtensions).toContain('init(hex: String)');
            const viewExtensions = await fs.readFile(path.join(sourcesDir, 'Extensions', 'ViewExtensions.swift'), 'utf-8');
        ...
        it('should set up SwiftUI component structure', async () => {
            ...
            const result = await builder.build(components, tokens, config);
            expect(result.success).toBe(true);
            const componentFile = await fs.readFile(path.join(testOutputDir, 'ios', 'Sources', 'DesignerPunk', 'Components', 'ButtonCTA.swift'), 'utf-8');
        ...
        it('should create Tests directory with test file', async () => {
            ...
            const testFile = await fs.readFile(path.join(testOutputDir, 'ios', 'Tests', 'DesignerPunkTests', 'DesignerPunkTests.swift'), 'utf-8');
    ...
    describe('build', () => {
        it('should build Swift Package with components and tokens', async () => {
            ...
            const tokensFile = await fs.readFile(path.join(iosDir, 'Sources', 'DesignerPunk', 'Tokens', 'Tokens.swift'), 'utf-8');
            expect(tokensFile).toContain('public enum Tokens');
        });
    ...
    describe('iOS build output validation (Task 3.4)', () => {
        ...
        it('should validate Swift constants compile correctly', async () => {
            ...
            const tokensFile = await fs.readFile(path.join(testOutputDir, 'ios', 'Sources', 'DesignerPunk', 'Tokens', 'Tokens.swift'), 'utf-8');
        ...
        it('should verify iOS-specific optimizations work', async () => {
            ...
            expect(result.success).toBe(true);
            // Read component file
            const componentFile = await fs.readFile(path.join(testOutputDir, 'ios', 'Sources', 'DesignerPunk', 'Components', 'OptimizedButton.swift'), 'utf-8');
...
//# sourceMappingURL=iOSBuilder.test.js.map