...
const CrossPlatformValidationReporter_1 = require("../CrossPlatformValidationReporter");
describe('CrossPlatformValidationReporter', () => {
    let reporter;
    beforeEach(() => {
        reporter = new CrossPlatformValidationReporter_1.CrossPlatformValidationReporter();
    });
    describe('generateReport', () => {
        it('should generate comprehensive report with all validation results', () => {
            ...
            // Act
            const report = reporter.generateReport(mathematicalResult, tokenComparisonResult, interfaceResults);
            // Assert
    ...
    describe('summarizeMathematicalConsistency', () => {
        ...
        it('should identify key findings from mathematical validation', () => {
            ...
            expect(report.mathematicalConsistency.keyFindings.some(f => f.includes('contrast ratio'))).toBe(true);
        });
    });
    ...
    describe('generateExecutiveSummary', () => {
        ...
        it('should provide next steps based on validation results', () => {
            ...
            expect(report.executiveSummary.nextSteps.some(step => step.includes('Re-run validation'))).toBe(true);
        });
    });
...
function createMockMathematicalResult(valid) {
    return {
        valid,
        ...
        metadata: {
            timestamp: new Date(),
            platformsValidated: ['ios', 'android', 'web'],
            tokensValidated: 10,
            validationDuration: 100
        }
    };
}
function createMockTokenComparisonResult(valid) {
    return {
        totalTokens: 10,
...
function createMockInterfaceResult(valid) {
    return {
        valid,
...
//# sourceMappingURL=CrossPlatformValidationReporter.test.js.map