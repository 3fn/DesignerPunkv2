...
class ErrorValidator {
     ...
     */
    validateMathematicalRelationships(token, mathematicalContext, options = {}) {
        ...
        if (!primitiveToken.mathematicalRelationship || primitiveToken.mathematicalRelationship.trim().length === 0) {
            return this.generateErrorResult(token, 'Missing mathematical relationship', `Primitive token ${primitiveToken.name} lacks required mathematical relationship definition`, 'mathematical-violation', {
        ...
        if (mathematicalContext?.expectedRelationship && mathematicalContext?.actualRelationship) {
            if (mathematicalContext.expectedRelationship !== mathematicalContext.actualRelationship) {
                return this.generateErrorResult(token, 'Mathematical relationship violation', 'Calculated mathematical relationship does not match expected relationship', 'mathematical-violation', {
        ...
        if (primitiveToken.baseValue <= 0) {
            return this.generateErrorResult(token, 'Invalid base value', `Base value ${primitiveToken.baseValue} must be positive`, 'mathematical-violation', {
        ...
        if (primitiveToken.familyBaseValue <= 0) {
            return this.generateErrorResult(token, 'Invalid family base value', `Family base value ${primitiveToken.familyBaseValue} must be positive`, 'mathematical-violation', {
     ...
     */
    validateBorderWidthMathematicalRelationships(token, options = {}) {
        ...
        if (primitiveToken.category !== types_1.TokenCategory.BORDER_WIDTH) {
            return null;
        }
        ...
        if (tokenName === 'borderWidth200') {
            ...
            if (primitiveToken.baseValue !== expectedValue) {
                return this.generateErrorResult(token, 'Border width mathematical relationship violation', `borderWidth200 must equal borderWidth100 × 2 (expected: ${expectedValue}, actual: ${primitiveToken.baseValue})`, 'mathematical-violation', {
                    expectedValue,
                    actualValue: primitiveToken.baseValue,
        ...
        if (tokenName === 'borderWidth400') {
            ...
            if (primitiveToken.baseValue !== expectedValue) {
                return this.generateErrorResult(token, 'Border width mathematical relationship violation', `borderWidth400 must equal borderWidth100 × 4 (expected: ${expectedValue}, actual: ${primitiveToken.baseValue})`, 'mathematical-violation', {
                    expectedValue,
                    actualValue: primitiveToken.baseValue,
        ...
        if (tokenName === 'borderWidth100') {
            if (primitiveToken.baseValue !== baseValue) {
                return this.generateErrorResult(token, 'Border width base value violation', `borderWidth100 must equal base value (expected: ${baseValue}, actual: ${primitiveToken.baseValue})`, 'mathematical-violation', {
     ...
     */
    validateBorderWidthSemanticReferences(token, registryContext, options = {}) {
        ...
        if (actualPrimitive !== expectedPrimitive) {
            return this.generateErrorResult(token, 'Border width semantic token reference violation', `${semanticToken.name} must reference ${expectedPrimitive} (actual: ${actualPrimitive || 'none'})`, 'mathematical-violation', {
        ...
        if (registryContext?.availablePrimitiveTokens) {
            ...
            if (!availablePrimitives.includes(expectedPrimitive)) {
                return this.generateErrorResult(token, 'Border width primitive token not found', `${semanticToken.name} references ${expectedPrimitive} which does not exist`, 'mathematical-violation', {
     ...
     */
    validateBaselineGridCompliance(token, mathematicalContext, options = {}) {
        // Only applies to primitive tokens
        if ('primitiveReferences' in token) {
            return null;
        }
        ...
        if (!isAligned) {
            ...
            return this.generateErrorResult(token, 'Baseline grid alignment violation', `Token value ${primitiveToken.baseValue} does not align with ${gridUnit}-unit baseline grid`, 'mathematical-violation', {
                actualValue: primitiveToken.baseValue,
        ...
        if (primitiveToken.baselineGridAlignment !== isAligned) {
            return this.generateErrorResult(token, 'Baseline grid alignment flag inconsistency', `Token baselineGridAlignment flag (${primitiveToken.baselineGridAlignment}) does not match actual alignment (${isAligned})`, 'mathematical-violation', {
     ...
     */
    validateCrossPlatformConsistency(token, mathematicalContext, options = {}) {
        ...
        if (crossPlatformData.failedPairs.length > 0) {
            return this.generateErrorResult(token, 'Cross-platform consistency violation', `Mathematical relationships not maintained across platforms: ${crossPlatformData.failedPairs.join(', ')}`, 'mathematical-violation', {
        ...
        if (crossPlatformData.maxDeviation > crossPlatformData.toleranceLevel) {
            return this.generateErrorResult(token, 'Cross-platform tolerance exceeded', `Maximum deviation ${crossPlatformData.maxDeviation.toFixed(4)} exceeds tolerance ${crossPlatformData.toleranceLevel.toFixed(4)}`, 'mathematical-violation', {
                platformInfo: crossPlatformData.values,
                expectedValue: crossPlatformData.toleranceLevel,
                actualValue: crossPlatformData.maxDeviation,
     ...
     */
    validateTokenReferences(token, registryContext, options = {}) {
        ...
        if (invalidReferences.length > 0) {
            return this.generateErrorResult(token, 'Invalid primitive token reference(s)', `Referenced primitive token(s) do not exist: ${invalidReferences.join(', ')}`, 'mathematical-violation', {
        ...
        if (selfReferences.length > 0) {
            return this.generateErrorResult(token, 'Self-reference violation', 'Semantic token cannot reference itself', 'mathematical-violation', {
     ...
     */
    validateCircularDependencies(token, registryContext, options = {}) {
        ...
        if (circularReferences.includes(tokenName)) {
            return this.generateErrorResult(token, 'Circular dependency detected', `Token ${tokenName} is part of a circular dependency chain`, 'mathematical-violation', {
     ...
     */
    validateFamilyFoundation(token, mathematicalContext, options = {}) {
        // Only applies to primitive tokens
        if ('primitiveReferences' in token) {
            return null;
        }
        ...
        if (primitiveToken.familyBaseValue !== familyFoundation.baseValue) {
            return this.generateErrorResult(token, 'Family base value inconsistency', `Token family base value ${primitiveToken.familyBaseValue} does not match family foundation ${familyFoundation.baseValue}`, 'mathematical-violation', {
     ...
     */
    generateErrorResult(token, message, rationale, scenario, contextData, suggestions) {
     ...
     */
    generateErrorAnalysisReport(contexts) {
        ...
        const baselineGridViolations = [];
        const crossPlatformViolations = [];
        const referenceViolations = [];
        const circularDependencies = [];
        const familyFoundationViolations = [];
    ...
    categorizeError(error) {
        if (error.message.includes('mathematical relationship'))
        ...
        if (error.message.includes('base value'))
            return 'base-value';
        return 'other';
    }
    ...
    generateSystemErrorRecommendations(errors) {
