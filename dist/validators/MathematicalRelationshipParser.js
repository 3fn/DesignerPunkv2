...
exports.MathematicalRelationshipParser = void 0;
/**
 * Mathematical Relationship Parser
 */
class MathematicalRelationshipParser {
    constructor() {
     ...
     */
    validate(expression, baseValue, familyBaseValue) {
        ...
        if (!mathematicallyCorrect) {
            errors.push(`Mathematical relationship incorrect: ${familyBaseValue} ${parsed.operator} ${parsed.operand} = ${calculatedResult}, but token baseValue is ${baseValue}`);
        }
        ...
        if (parsed.expectedResult !== undefined) {
            ...
            if (!expectedMatchesCalculated && !expectedMatchesActual) {
                errors.push(`Expected result ${parsed.expectedResult} does not match calculated result ${calculatedResult} or actual baseValue ${baseValue}`);
            }
        }
        return {
            isValid: errors.length === 0,
     ...
     */
    areEquivalent(expr1, expr2, familyBaseValue) {
     ...
     */
    normalize(expression, baseValue, familyBaseValue) {
        const parsed = this.parse(expression);
        if (!parsed.isValid) {
     ...
     */
    generate(baseValue, familyBaseValue) {
...
exports.MathematicalRelationshipParser = MathematicalRelationshipParser;
//# sourceMappingURL=MathematicalRelationshipParser.js.map