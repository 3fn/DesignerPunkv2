...
const Icon_web_1 = require("../platforms/web/Icon.web");
describe('Icon Component', () => {
    ...
    describe('Core Rendering', () => {
        it('renders SVG with correct size attributes', () => {
            const result = (0, Icon_web_1.createIcon)({ name: 'arrow-right', size: 24 });
        ...
        it('renders all available icons', () => {
            ...
            iconNames.forEach(name => {
                const result = (0, Icon_web_1.createIcon)({ name: name, size: 24 });
                expect(result).toContain('<svg');
                expect(result).toContain(`icon-${name}`);
            });
        });
        it('handles invalid icon names with fallback', () => {
            const result = (0, Icon_web_1.createIcon)({
                name: 'invalid-icon',
                size: 24
            });
    ...
    describe('Color Inheritance and Override', () => {
        it('uses currentColor by default for color inheritance', () => {
            const result = (0, Icon_web_1.createIcon)({ name: 'check', size: 24 });
    ...
    describe('Accessibility', () => {
        ...
        it('works in button context without interfering with label', () => {
            const iconHTML = (0, Icon_web_1.createIcon)({ name: 'check', size: 24 });
            const buttonHTML = `
        <button>
          ${iconHTML}
          <span>Submit</span>
    ...
    describe('Icon Class API', () => {
        it('creates Icon instance and renders', () => {
            const icon = new Icon_web_1.Icon({ name: 'check', size: 24 });
        ...
        it('gets current properties', () => {
            const icon = new Icon_web_1.Icon({
                name: 'check',
                size: 24,
                className: 'test'
            });
    ...
    describe('Size Variants', () => {
        it('accepts all IconSize values from icon size tokens', () => {
            const allSizes = [13, 18, 24, 28, 32, 36, 40, 44, 48];
            allSizes.forEach(size => {
                const result = (0, Icon_web_1.createIcon)({ name: 'check', size });
        ...
        it('renders core sizes correctly (90% use cases)', () => {
            const coreSizes = [18, 24, 32, 36, 40];
            coreSizes.forEach(size => {
                const result = (0, Icon_web_1.createIcon)({ name: 'arrow-right', size });
                expect(result).toContain(`width="${size}"`);
                expect(result).toContain(`height="${size}"`);
        ...
        it('renders available sizes correctly (10% use cases)', () => {
            const availableSizes = [13, 28, 44, 48];
            availableSizes.forEach(size => {
                const result = (0, Icon_web_1.createIcon)({ name: 'heart', size });
                expect(result).toContain(`width="${size}"`);
                expect(result).toContain(`height="${size}"`);
        ...
        it('maintains type safety with IconSize type', () => {
            ...
            const validSize = 24;
            const result = (0, Icon_web_1.createIcon)({ name: 'check', size: validSize });
            expect(result).toContain('width="24"');
