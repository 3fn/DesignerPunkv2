...
const createSpacingToken = (name, value, isStrategicFlexibility = false) => createMockToken({
    name,
    category: PrimitiveToken_1.TokenCategory.SPACING,
    baseValue: value,
...
const createFontSizeToken = (name, value) => createMockToken({
    name,
...
describe('PrimitiveTokenRegistry', () => {
    let registry;
    beforeEach(() => {
        registry = new PrimitiveTokenRegistry_1.PrimitiveTokenRegistry();
    });
    ...
    describe('Token Querying', () => {
        ...
        test('should filter tokens by category', () => {
            const spacingTokens = registry.query({ category: PrimitiveToken_1.TokenCategory.SPACING });
            const fontSizeTokens = registry.query({ category: PrimitiveToken_1.TokenCategory.FONT_SIZE });
            expect(spacingTokens).toHaveLength(3);
            expect(fontSizeTokens).toHaveLength(2);
            expect(spacingTokens.every(token => token.category === PrimitiveToken_1.TokenCategory.SPACING)).toBe(true);
        });
        ...
        test('should sort tokens by category', () => {
            ...
            expect(sortedTokens[0].category).toBe(PrimitiveToken_1.TokenCategory.FONT_SIZE);
            expect(sortedTokens[1].category).toBe(PrimitiveToken_1.TokenCategory.FONT_SIZE);
            expect(sortedTokens[2].category).toBe(PrimitiveToken_1.TokenCategory.SPACING);
        });
    });
    describe('Category Organization', () => {
        ...
        test('should organize tokens by category', () => {
            const spacingTokens = registry.getByCategory(PrimitiveToken_1.TokenCategory.SPACING);
            const fontSizeTokens = registry.getByCategory(PrimitiveToken_1.TokenCategory.FONT_SIZE);
        ...
        test('should return empty array for categories with no tokens', () => {
            const radiusTokens = registry.getByCategory(PrimitiveToken_1.TokenCategory.RADIUS);
    ...
    describe('Registry Statistics', () => {
        ...
        test('should provide category statistics', () => {
            const stats = registry.getStats();
            expect(stats.categoryStats[PrimitiveToken_1.TokenCategory.SPACING]).toBe(2);
            expect(stats.categoryStats[PrimitiveToken_1.TokenCategory.FONT_SIZE]).toBe(1);
            expect(stats.categoryStats[PrimitiveToken_1.TokenCategory.RADIUS]).toBe(0);
        });
        test('should not include validation information (validation removed from registry)', () => {
            const stats = registry.getStats();
            expect(stats.validationInfo).toBeUndefined();
        });
    });
    describe('Token Removal and Cleanup', () => {
        ...
        test('should remove existing tokens successfully', () => {
            ...
            expect(registry.getByCategory(PrimitiveToken_1.TokenCategory.SPACING)).toHaveLength(0);
        });
        ...
        test('should clear all tokens and reset categories', () => {
            ...
            expect(registry.getByCategory(PrimitiveToken_1.TokenCategory.SPACING)).toHaveLength(0);
            expect(registry.getByCategory(PrimitiveToken_1.TokenCategory.FONT_SIZE)).toHaveLength(0);
        });
...
//# sourceMappingURL=PrimitiveTokenRegistry.test.js.map