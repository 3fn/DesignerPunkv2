...
class VersionCalculator {
     ...
     */
    calculateVersionBump(changes, currentVersion) {
     ...
     */
    validateSemanticVersioning(recommendation) {
        const errors = [];
        const warnings = [];
     ...
     */
    generateVersionRationale(changes) {
        const rationale = [];
     ...
     */
    handlePreReleaseVersions(currentVersion, bumpType) {
     ...
     */
    calculateNewVersion(parsed, bumpType) {
        switch (bumpType) {
            case 'major':
     ...
     */
    calculateConfidence(changes, bumpType) {
     ...
     */
    analyzePreReleaseInfo(parsed, bumpType) {
        if (!parsed.preRelease) {
            return undefined;
        }
        const preReleaseMatch = parsed.preRelease.match(/^(alpha|beta|rc)\.?(\d+)?$/);
     ...
     */
    determineBreakingChangeImpact(breakingChanges) {
     ...
     */
    determineFeatureImpact(features) {
     ...
     */
    determineBugFixImpact(bugFixes) {
     ...
     */
    determineImprovementImpact(improvements) {
...
//# sourceMappingURL=VersionCalculator.js.map