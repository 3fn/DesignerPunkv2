...
exports.PerformanceBenchmarkRunner = void 0;
...
class PerformanceBenchmarkRunner {
    constructor(workingDirectory, analysisConfig, config = {}) {
        this.testDocuments = new Map();
        this.workingDirectory = workingDirectory;
        this.analysisConfig = analysisConfig;
        this.config = {
            outputDir: (0, path_1.join)(workingDirectory, 'benchmark-results'),
            iterations: 5,
            warmupIterations: 2,
    ...
    async runBenchmarkTest(config) {
        ...
        const analyzer = new PerformanceOptimizedAnalyzer_1.PerformanceOptimizedAnalyzer(this.workingDirectory, this.analysisConfig, {
        ...
        const documentCache = new DocumentParsingCache_1.DocumentParsingCache(this.workingDirectory, {
        ...
        const parallelProcessor = new ParallelProcessor_1.ParallelProcessor({
        ...
        for (let i = 0; i < this.config.iterations; i++) {
            try {
                const iterationResult = await this.executeBenchmarkIteration(documents, documentCache, parallelProcessor);
                iterations.push({
     ...
     */
    async executeBenchmarkIteration(documents, documentCache, parallelProcessor) {
     ...
     */
    getTestDocuments(testName, count, sizeBytes) {
     ...
     */
    generateDocumentContent(sizeBytes, index, testName) {
...
`;
    ...
    async detectRegressions(results) {
        const regressions = [];
        if (!this.config.baselineFile || !(0, fs_1.existsSync)(this.config.baselineFile)) {
            return regressions;
        }
        try {
            const baselineData = JSON.parse(require('fs').readFileSync(this.config.baselineFile, 'utf8'));
            for (const result of results) {
                const baselineResult = baselineData.results?.find((r) => r.config.name === result.config.name);
                if (baselineResult) {
     ...
     */
    async saveResults(suiteResult) {
        const timestamp = suiteResult.timestamp.toISOString().replace(/[:.]/g, '-');
        const filename = `benchmark-results-${timestamp}.json`;
        const filepath = (0, path_1.join)(this.config.outputDir, filename);
        try {
            (0, fs_1.writeFileSync)(filepath, JSON.stringify(suiteResult, null, 2));
            // Also save as latest baseline
            const baselineFile = (0, path_1.join)(this.config.outputDir, 'baseline.json');
            (0, fs_1.writeFileSync)(baselineFile, JSON.stringify(suiteResult, null, 2));
            if (this.config.enableDetailedLogging) {
...
exports.PerformanceBenchmarkRunner = PerformanceBenchmarkRunner;
//# sourceMappingURL=PerformanceBenchmarkRunner.js.map