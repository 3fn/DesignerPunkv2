...
const PerformanceOptimizedAnalyzer_1 = require("../PerformanceOptimizedAnalyzer");
...
describe('Performance Benchmarks', () => {
    ...
    beforeAll(async () => {
        ...
        testDir = (0, path_1.join)((0, os_1.tmpdir)(), `release-analysis-perf-${Date.now()}`);
        ...
        gitOptimizer = new GitPerformanceOptimizer_1.GitPerformanceOptimizer(testDir, {
            maxCommitsPerBatch: 100,
            useGitCache: true,
            parallelFileProcessing: true,
            maxConcurrentOperations: 4
        });
        documentCache = new DocumentParsingCache_1.DocumentParsingCache(testDir, {
            enableCache: true,
            maxCacheSize: 1000,
            enableParallelParsing: true,
            maxConcurrentParsing: 4
        });
        parallelProcessor = new ParallelProcessor_1.ParallelProcessor({
            maxConcurrency: 4,
            batchSize: 10,
            enableProgressReporting: false, // Disable for benchmarks
            enableErrorRecovery: true
        });
        analyzer = new PerformanceOptimizedAnalyzer_1.PerformanceOptimizedAnalyzer(testDir, analysisConfig, {
    ...
    describe('Memory Usage Benchmarks', () => {
        ...
        it('should maintain reasonable memory usage under load', async () => {
            ...
            const memoryGrowthRate = calculateGrowthRate(memoryResults.map(r => r.count), memoryResults.map(r => r.peak));
    ...
    describe('Scalability Testing', () => {
        ...
        it('should identify optimal concurrency level', async () => {
            ...
            const optimalResult = concurrencyResults.reduce((best, current) => current.efficiency > best.efficiency ? current : best);
    ...
    describe('Regression Testing', () => {
        ...
        it('should show improvement with optimizations enabled', async () => {
            ...
            const unoptimizedAnalyzer = new PerformanceOptimizedAnalyzer_1.PerformanceOptimizedAnalyzer(testDir, analysisConfig, {
    ...
    async function runRepositoryBenchmark(config) {
    ...
    async function runScalabilityTest(documentCounts) {
    ...
    async function benchmarkAnalyzerPerformance(analyzer, documents) {
    ...
    function generateTestDocuments(count, sizeBytes) {
        const documents = [];
    ...
    function generateDocumentContent(sizeBytes, index) {
...
`;
    ...
    function setupMockGitRepository(documents) {
        // Create directory structure
        for (const doc of documents) {
            const dir = (0, path_1.join)(testDir, doc.path.split('/').slice(0, -1).join('/'));
            (0, fs_1.mkdirSync)(dir, { recursive: true });
            (0, fs_1.writeFileSync)((0, path_1.join)(testDir, doc.path), doc.content);
        }
    }
    ...
    function calculateGrowthRate(inputs, outputs) {
    ...
    function loadPerformanceBaseline(config) {
        // In a real implementation, this would load from a file or database
        // For testing, return null to indicate no baseline exists
        return null;
    }
    function storePerformanceBaseline(config, baseline) {
...
//# sourceMappingURL=PerformanceBenchmarks.test.js.map