...
describe('Release Detection Workflow', () => {
    ...
    const releaseManagerPath = (0, path_1.join)(projectRoot, '.kiro/hooks/release-manager.sh');
    const workflowDocPath = (0, path_1.join)(projectRoot, '.kiro/steering/Development Workflow.md');
    describe('Release Manager Script', () => {
        it('should exist and be executable', async () => {
            const stats = await fs_1.promises.stat(releaseManagerPath);
            expect(stats.isFile()).toBe(true);
            expect(stats.mode & 0o111).toBeTruthy(); // Check executable bit
        });
        it('should support auto mode for automatic detection', async () => {
            const content = await fs_1.promises.readFile(releaseManagerPath, 'utf-8');
            // Should support 'auto' argument for automatic detection
            expect(content).toContain('auto');
        });
        it('should be documented in workflow', async () => {
            const workflowContent = await fs_1.promises.readFile(workflowDocPath, 'utf-8');
    ...
    describe('Automatic Hook Configuration', () => {
        const autoHookPath = (0, path_1.join)(projectRoot, '.kiro/hooks/release-detection-auto.kiro.hook');
        it('should exist', async () => {
            const stats = await fs_1.promises.stat(autoHookPath);
            expect(stats.isFile()).toBe(true);
        });
        it('should be valid JSON', async () => {
            const content = await fs_1.promises.readFile(autoHookPath, 'utf-8');
    ...
    describe('Manual Hook Configuration', () => {
        const manualHookPath = (0, path_1.join)(projectRoot, '.kiro/hooks/release-detection-manual.kiro.hook');
        it('should exist', async () => {
            const stats = await fs_1.promises.stat(manualHookPath);
            expect(stats.isFile()).toBe(true);
        });
        it('should be valid JSON', async () => {
            const content = await fs_1.promises.readFile(manualHookPath, 'utf-8');
            const config = JSON.parse(content);
            expect(config).toBeDefined();
        });
    ...
    describe('Current Implementation', () => {
        ...
        it('should have hook configurations for both automatic and manual triggers', async () => {
            ...
            const autoExists = await fs_1.promises.access(autoHookPath).then(() => true).catch(() => false);
            const manualExists = await fs_1.promises.access(manualHookPath).then(() => true).catch(() => false);
        ...
        it('should document why automatic hook files were not implemented', async () => {
            ...
            const analyzeHookPath = (0, path_1.join)(projectRoot, '.kiro/hooks/analyze-after-commit.sh');
            const exists = await fs_1.promises.access(analyzeHookPath).then(() => true).catch(() => false);
