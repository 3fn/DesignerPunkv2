...
exports.ReleaseCLI = void 0;
exports.runAnalysisCli = main;
...
class ReleaseCLI {
    ...
    async analyzeChanges(options = {}) {
        ...
        const recoveryUtils = (0, ErrorRecovery_1.createRecoveryUtilities)(this.workingDirectory);
        const result = await (0, ErrorHandler_1.withErrorHandling)(async () => {
            ...
            try {
                ...
                const extractor = new (await Promise.resolve().then(() => __importStar(require('../extraction/SimpleChangeExtractor')))).SimpleChangeExtractor(config.extraction);
...
`;
     ...
     */
    async confirmVersionBump(recommendation) {
     ...
     */
    async saveAnalysis(result, filePath) {
        try {
            const analysisData = {
                timestamp: new Date().toISOString(),
                analysis: result
            };
            await fs_1.promises.writeFile(filePath, JSON.stringify(analysisData, null, 2), 'utf-8');
    ...
    async loadConfiguration() {
        ...
        const result = await (0, ErrorHandler_1.withErrorHandling)(async () => {
            ...
            const { AnalysisConfigManager } = await Promise.resolve().then(() => __importStar(require('../config/AnalysisConfigManager')));
            ...
            try {
                ...
                const { DEFAULT_ANALYSIS_CONFIG } = await Promise.resolve().then(() => __importStar(require('../config/AnalysisConfig')));
    ...
    async findAllCompletionDocuments() {
        ...
        const result = await (0, ErrorHandler_1.withErrorHandling)(async () => {
            ...
            try {
                const collector = new (await Promise.resolve().then(() => __importStar(require('../collection/CompletionDocumentCollector')))).CompletionDocumentCollector(this.workingDirectory, await this.loadConfiguration());
     ...
     */
    async performSimpleDocumentScan() {
        const documents = [];
     ...
     */
    async performBasicExtraction(documents) {
        const features = [];
        const bugFixes = [];
        const improvements = [];
...
//# sourceMappingURL=ReleaseCLI.js.map