...
const AccuracyValidationFramework_1 = require("../AccuracyValidationFramework");
const AccuracyTestCases_1 = require("../AccuracyTestCases");
const AnalysisConfig_1 = require("../../config/AnalysisConfig");
describe('AccuracyValidationFramework', () => {
    let framework;
    beforeEach(() => {
        framework = new AccuracyValidationFramework_1.AccuracyValidationFramework(AnalysisConfig_1.DEFAULT_ANALYSIS_CONFIG.extraction);
    });
    describe('Test Case Registration', () => {
        it('should register and retrieve test cases', () => {
            const testCase = AccuracyTestCases_1.AccuracyTestCases.getStructuredBreakingChangeTestCase();
        ...
        it('should register multiple test cases', () => {
            const testCases = [
                AccuracyTestCases_1.AccuracyTestCases.getStructuredBreakingChangeTestCase(),
                AccuracyTestCases_1.AccuracyTestCases.getStructuredFeatureTestCase(),
                AccuracyTestCases_1.AccuracyTestCases.getStructuredBugFixTestCase()
            ];
    ...
    describe('Single Test Case Validation', () => {
        ...
        it('should validate structured feature document accurately', async () => {
            const testCase = AccuracyTestCases_1.AccuracyTestCases.getStructuredFeatureTestCase();
            framework.registerTestCase(testCase);
            const result = await framework.validateTestCase(testCase.id);
            expect(result.passed).toBe(true);
            expect(result.overallAccuracy).toBeGreaterThan(0.6);
        ...
        it('should validate structured bug fix document accurately', async () => {
            const testCase = AccuracyTestCases_1.AccuracyTestCases.getStructuredBugFixTestCase();
            framework.registerTestCase(testCase);
            const result = await framework.validateTestCase(testCase.id);
            expect(result.passed).toBe(true);
            expect(result.overallAccuracy).toBeGreaterThan(0.6);
        ...
        it('should handle unstructured mixed document with lower confidence', async () => {
            const testCase = AccuracyTestCases_1.AccuracyTestCases.getUnstructuredMixedTestCase();
            framework.registerTestCase(testCase);
            const result = await framework.validateTestCase(testCase.id);
        ...
        it('should handle edge case empty document', async () => {
            const testCase = AccuracyTestCases_1.AccuracyTestCases.getEdgeCaseEmptyTestCase();
            framework.registerTestCase(testCase);
            const result = await framework.validateTestCase(testCase.id);
            expect(result.passed).toBe(true);
        ...
        it('should handle documentation-only document', async () => {
            const testCase = AccuracyTestCases_1.AccuracyTestCases.getEdgeCaseDocumentationOnlyTestCase();
            framework.registerTestCase(testCase);
            const result = await framework.validateTestCase(testCase.id);
    ...
    describe('Version Bump Validation', () => {
        it('should validate major version bump correctly', async () => {
            const testCase = AccuracyTestCases_1.AccuracyTestCases.getVersionBumpMajorTestCase();
            framework.registerTestCase(testCase);
            const result = await framework.validateTestCase(testCase.id);
            expect(result.versionBumpAccuracy.correctBumpType).toBe(true);
            expect(result.details.versionCalculation.bumpType).toBe('major');
        ...
        it('should validate minor version bump correctly', async () => {
            const testCase = AccuracyTestCases_1.AccuracyTestCases.getVersionBumpMinorTestCase();
            framework.registerTestCase(testCase);
            const result = await framework.validateTestCase(testCase.id);
            expect(result.versionBumpAccuracy.correctBumpType).toBe(true);
            expect(result.details.versionCalculation.bumpType).toBe('minor');
        ...
        it('should validate patch version bump correctly', async () => {
            const testCase = AccuracyTestCases_1.AccuracyTestCases.getVersionBumpPatchTestCase();
            framework.registerTestCase(testCase);
            const result = await framework.validateTestCase(testCase.id);
            expect(result.versionBumpAccuracy.correctBumpType).toBe(true);
            expect(result.details.versionCalculation.bumpType).toBe('patch');
        ...
        it('should validate no version bump correctly', async () => {
            const testCase = AccuracyTestCases_1.AccuracyTestCases.getVersionBumpNoneTestCase();
            framework.registerTestCase(testCase);
            const result = await framework.validateTestCase(testCase.id);
            expect(result.versionBumpAccuracy.correctBumpType).toBe(true);
            expect(result.details.versionCalculation.bumpType).toBe('none');
    ...
    describe('Confidence Validation', () => {
        it('should validate high confidence scenario', async () => {
            const testCase = AccuracyTestCases_1.AccuracyTestCases.getHighConfidenceTestCase();
            framework.registerTestCase(testCase);
            const result = await framework.validateTestCase(testCase.id);
        ...
        it('should handle low confidence scenario appropriately', async () => {
            const testCase = AccuracyTestCases_1.AccuracyTestCases.getLowConfidenceTestCase();
            framework.registerTestCase(testCase);
            const result = await framework.validateTestCase(testCase.id);
    ...
    describe('Accuracy Metrics by Category', () => {
        it('should calculate accuracy metrics grouped by test case category', async () => {
            const testCases = [
                AccuracyTestCases_1.AccuracyTestCases.getStructuredBreakingChangeTestCase(), // structured
                AccuracyTestCases_1.AccuracyTestCases.getUnstructuredMixedTestCase(), // unstructured
                AccuracyTestCases_1.AccuracyTestCases.getEdgeCaseEmptyTestCase() // edge-case
            ];
            framework.registerTestCases(testCases);
            const results = await framework.validateAllTestCases();
    ...
    describe('Integration with Real Extraction System', () => {
        it('should work with actual SimpleChangeExtractor and VersionCalculator', async () => {
            const testCase = AccuracyTestCases_1.AccuracyTestCases.getComplexStructuredTestCase();
            framework.registerTestCase(testCase);
            const result = await framework.validateTestCase(testCase.id);
...
//# sourceMappingURL=AccuracyValidationFramework.test.js.map