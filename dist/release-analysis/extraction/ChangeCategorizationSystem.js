...
class ChangeCategorizationSystem {
    constructor(config) {
     ...
     */
    categorizeChange(title, description, context) {
     ...
     */
    assessBreakingChangeSeverity(title, description, affectedAPIs) {
     ...
     */
    classifyFeature(title, description, artifacts) {
        const fullText = `${title} ${description}`.toLowerCase();
     ...
     */
    classifyImprovement(title, description) {
        const fullText = `${title} ${description}`.toLowerCase();
        let bestType = 'other';
        let bestConfidence = 0.3;
        let impact = 'medium';
     ...
     * Classify a bug fix
     */
    classifyBugFix(title, description, affectedComponents) {
        const fullText = `${title} ${description}`.toLowerCase();
        let severity = 'medium';
        let category = 'other';
        let confidence = 0.5;
     ...
     */
    checkDocumentationPatterns(text) {
        const docPatterns = [
     ...
     */
    generateSeverityReasoning(severity, indicators, apiCount) {
        const reasons = [];
     ...
     */
    categorizeByArtifacts(artifacts) {
     ...
     */
    assessImprovementImpact(text, reasoning) {
...
//# sourceMappingURL=ChangeCategorizationSystem.js.map