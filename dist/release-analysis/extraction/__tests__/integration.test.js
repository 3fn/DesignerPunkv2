...
const ChangeExtractor_1 = require("../ChangeExtractor");
const AnalysisConfig_1 = require("../../config/AnalysisConfig");
describe('Change Extraction Integration', () => {
    let extractor;
    beforeEach(() => {
        extractor = new ChangeExtractor_1.DefaultChangeExtractor(AnalysisConfig_1.DEFAULT_ANALYSIS_CONFIG.extraction);
    });
...
`,
        ...
        expect(result.breakingChanges.some(change => change.title.toLowerCase().includes('tokenvalidator') ||
            change.title.toLowerCase().includes('buildconfig'))).toBe(true);
        expect(result.newFeatures.some(feature => feature.title.toLowerCase().includes('mathematical') ||
            feature.title.toLowerCase().includes('platform'))).toBe(true);
        expect(result.bugFixes.some(fix => fix.title.toLowerCase().includes('spacing') ||
            fix.title.toLowerCase().includes('unit conversion'))).toBe(true);
        // Verify metadata
