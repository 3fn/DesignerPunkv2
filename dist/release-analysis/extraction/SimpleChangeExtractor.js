...
class SimpleChangeExtractor {
     ...
     */
    parseStructuredDocument(document, sectionMatches) {
     ...
     */
    async parseUnstructuredDocument(document) {
     ...
     */
    extractBreakingChangesFromSection(section, document) {
        const changes = [];
     ...
     */
    extractFeaturesFromSection(section, document) {
        const features = [];
        const items = this.extractListItems(section.content);
        for (const item of items) {
     ...
     */
    extractBugFixesFromSection(section, document) {
        const bugFixes = [];
        const items = this.extractListItems(section.content);
        for (const item of items) {
     ...
     */
    extractImprovementsFromSection(section, document) {
        const improvements = [];
        const items = this.extractListItems(section.content);
        for (const item of items) {
            improvements.push({
     ...
     * Extract mixed changes from summary section
     */
    extractMixedChangesFromSection(section, document) {
     ...
     */
    createChangeFromPattern(match, document) {
        const baseItem = {
            id: generateId(),
     ...
     */
    async extractBreakingChangesWithStructure(document) {
        const breakingChanges = [];
     ...
     */
    async extractFeaturesWithStructure(document) {
        const features = [];
        const content = document.content;
        // Prioritized extraction strategy: structured sections first
     ...
     */
    async extractBugFixesWithStructure(document) {
        const bugFixes = [];
        const content = document.content;
    ...
    semanticDeduplicateBreakingChanges(changes) {
        return this.semanticDeduplicate(changes, (a, b) => this.calculateSimilarity(a.title, b.title) > this.config.confidenceThresholds.semanticSimilarityThreshold ||
    ...
    semanticDeduplicateFeatures(features) {
        return this.semanticDeduplicate(features, (a, b) => this.calculateSimilarity(a.title, b.title) > this.config.confidenceThresholds.semanticSimilarityThreshold ||
            this.calculateSimilarity(a.description, b.description) > this.config.confidenceThresholds.semanticSimilarityThreshold);
    }
    semanticDeduplicateBugFixes(bugFixes) {
        return this.semanticDeduplicate(bugFixes, (a, b) => this.calculateSimilarity(a.title, b.title) > this.config.confidenceThresholds.semanticSimilarityThreshold ||
            this.calculateSimilarity(a.description, b.description) > this.config.confidenceThresholds.semanticSimilarityThreshold);
    }
    semanticDeduplicateImprovements(improvements) {
        return this.semanticDeduplicate(improvements, (a, b) => this.calculateSimilarity(a.title, b.title) > this.config.confidenceThresholds.semanticSimilarityThreshold ||
            this.calculateSimilarity(a.description, b.description) > this.config.confidenceThresholds.semanticSimilarityThreshold);
    }
    /**
     * Generic semantic deduplication with similarity function
     */
    semanticDeduplicate(items, similarityFn) {
        if (items.length <= 1)
            return items;
        const deduplicated = [];
     ...
     */
    assessBreakingChangeSeverity(text, affectedAPIs = []) {
        const assessment = this.categorizationSystem.assessBreakingChangeSeverity(this.extractTitle(text), text, affectedAPIs);
    ...
    extractBenefits(text) {
        ...
        const classification = this.categorizationSystem.classifyFeature(this.extractTitle(text), text, []);
     ...
     */
    categorizeFeature(text, artifacts = []) {
        const classification = this.categorizationSystem.classifyFeature(this.extractTitle(text), text, artifacts);
        return classification.category;
    }
    /**
     * Extract issue number from text
     */
    extractIssueNumber(text) {
     ...
     */
    assessBugFixSeverity(text, affectedComponents = []) {
        const classification = this.categorizationSystem.classifyBugFix(this.extractTitle(text), text, affectedComponents);
        return classification.severity;
    }
    /**
     * Categorize improvement type using categorization system
     */
    categorizeImprovement(text) {
        const classification = this.categorizationSystem.classifyImprovement(this.extractTitle(text), text);
        return classification.type;
    }
    /**
     * Assess improvement impact using categorization system
     */
    assessImprovementImpact(text) {
        const classification = this.categorizationSystem.classifyImprovement(this.extractTitle(text), text);
     ...
     */
    buildDeduplicationMetadata(results) {
        ...
        for (const { type, result } of results) {
            for (const uncertain of result.uncertainDuplicates) {
                uncertainDuplicates.push({
                    ...
                    items: uncertain.items.map((item) => ({
                        id: item.id,
                        title: item.title,
                        source: item.source
                    }))
     ...
     */
    calculateConfidenceWithDeduplication(changes, deduplicationMetadata) {
     ...
     */
    calculateConfidenceMetrics(changes, documents, patternMatches, sectionMatches) {
     ...
     */
    validateExtractionWithConfidence(changes, documents, patternMatches, sectionMatches) {
     ...
     */
    getCategorizationInsights(changes) {
     ...
     */
    async extractChangesWithConfidence(documents) {
     ...
     */
    async extractChangesWithTracking(documents, patternMatches, sectionMatches) {
        const allChanges = [];
        const filteredDocuments = [];
        for (const document of documents) {
            // Check if document should be excluded
     ...
     */
    async parseCompletionDocumentWithTracking(document, patternMatches, sectionMatches) {
        // Try structured section parsing first
     ...
     */
    parseUnstructuredDocumentWithPatterns(document, patternMatches) {
        const breakingChanges = [];
        const newFeatures = [];
        const bugFixes = [];
        const improvements = [];
...
//# sourceMappingURL=SimpleChangeExtractor.js.map