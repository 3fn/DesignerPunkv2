...
describe('AnalysisReporter', () => {
    ...
    beforeEach(() => {
        reporter = new AnalysisReporter_1.AnalysisReporter();
    ...
    describe('saveReport', () => {
        ...
        it('should save summary report to file', async () => {
            ...
            expect(mockFs.writeFile).toHaveBeenCalledWith('/tmp/report.txt', expect.stringContaining('📊 Release Analysis Summary'), 'utf-8');
        });
        it('should save detailed report to file', async () => {
            ...
            expect(mockFs.writeFile).toHaveBeenCalledWith('/tmp/detailed-report.txt', expect.stringContaining('📊 Detailed Analysis Report'), 'utf-8');
        });
        it('should save JSON report to file', async () => {
            ...
            expect(mockFs.writeFile).toHaveBeenCalledWith('/tmp/report.json', expect.stringMatching(/^\{[\s\S]*\}$/), // Valid JSON structure
            'utf-8');
        });
        it('should include timestamp when requested', async () => {
            ...
            expect(mockFs.writeFile).toHaveBeenCalledWith('/tmp/report.txt', expect.stringContaining('Generated:'), 'utf-8');
        });
        it('should handle file system errors gracefully', async () => {
            ...
            await expect(reporter.saveReport(mockAnalysisResult, format, '/tmp/report.txt')).rejects.toThrow('Permission denied');
        });
        it('should throw error for unsupported format', async () => {
            ...
            await expect(reporter.saveReport(mockAnalysisResult, format, '/tmp/report.txt')).rejects.toThrow('Unsupported report format: unsupported');
        });
    });
...
//# sourceMappingURL=AnalysisReporter.test.js.map