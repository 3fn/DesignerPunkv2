 ...
 */
class ReleaseAnalysisErrorHandler {
    constructor() {
        this.errorHistory = [];
     ...
     */
    async handleGitError(error, context, fallbackAction) {
        const errorDetails = {
            code: 'GIT_ERROR',
     ...
     */
    async handleParsingError(error, filePath, context, skipOnError = true) {
        const errorDetails = {
            code: 'PARSING_ERROR',
     ...
     */
    async handleValidationError(error, context, confidenceImpact = 0.3) {
        const errorDetails = {
     ...
     * Requirement 8.5: Clear, actionable error messages with resolution guidance
     */
    async handleConfigurationError(error, context, configPath) {
        const errorDetails = {
            code: 'CONFIG_ERROR',
     ...
     */
    async handleFilesystemError(error, filePath, context, operation = 'read') {
        const errorDetails = {
    ...
    createErrorSummary() {
        ...
        const recoverableErrors = this.errorHistory.filter(e => e.severity !== 'critical' && e.recoveryStrategies.length > 0);
    ...
    determineGitErrorSeverity(error) {
        const message = error.message.toLowerCase();
        if (message.includes('not a git repository')) {
            return 'critical';
        }
    ...
    getGitRecoveryStrategies(error, fallbackAction) {
        const message = error.message.toLowerCase();
        const strategies = [];
        if (message.includes('not a git repository')) {
            strategies.push({
    ...
    extractParsingErrorMessage(error) {
    ...
    getParsingRecoveryStrategies(filePath, skipOnError) {
        const strategies = [];
        if (skipOnError) {
            strategies.push({
                type: 'skip',
    ...
    getParsingUserGuidance(error, filePath) {
    ...
    getValidationRecoveryStrategies(confidenceImpact) {
        return [{
                type: 'fallback',
    ...
    getConfigurationUserGuidance(error, configPath) {
        if (configPath) {
    ...
    extractFilesystemErrorMessage(error) {
        const message = error.message.toLowerCase();
    ...
    getFilesystemRecoveryStrategies(filePath, operation) {
        const strategies = [];
        if (operation === 'read') {
            strategies.push({
                type: 'skip',
    ...
    getFilesystemUserGuidance(error, filePath, operation) {
        const message = error.message.toLowerCase();
 ...
 */
async function withErrorHandling(operation, context, errorHandler = exports.releaseAnalysisErrorHandler) {
    try {
