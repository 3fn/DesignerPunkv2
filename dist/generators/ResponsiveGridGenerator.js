...
exports.ResponsiveGridGenerator = exports.responsiveGridConfigs = void 0;
const BreakpointTokens_1 = require("../tokens/BreakpointTokens");
...
class ResponsiveGridGenerator {
    ...
    generateResponsiveGridCSS() {
        ...
        const breakpointsWithMediaQueries = exports.responsiveGridConfigs.slice(1);
     ...
     */
    generateBaseGridStyles() {
        const xsConfig = exports.responsiveGridConfigs[0]; // xs breakpoint
        return [
     ...
     */
    generateMediaQuery(config) {
        const breakpointToken = BreakpointTokens_1.breakpointTokens[config.breakpoint];
        if (!breakpointToken) {
    ...
    generateGridItemStyles() {
        ...
        for (const config of exports.responsiveGridConfigs) {
            const label = config.breakpoint.replace('breakpoint', '').toLowerCase();
            const breakpointToken = BreakpointTokens_1.breakpointTokens[config.breakpoint];
            const breakpointValue = breakpointToken.platforms.web.value;
            const breakpointUnit = breakpointToken.platforms.web.unit;
    ...
    validateConfiguration() {
        ...
        for (const config of exports.responsiveGridConfigs) {
            ...
            if (!BreakpointTokens_1.breakpointTokens[config.breakpoint]) {
                errors.push(`Breakpoint token '${config.breakpoint}' not found`);
            }
            // Validate grid spacing tokens exist
            if (!GridSpacingTokens_1.gridSpacingTokens[config.gutter]) {
                errors.push(`Grid gutter token '${config.gutter}' not found`);
            }
            if (!GridSpacingTokens_1.gridSpacingTokens[config.margin]) {
        ...
        for (let i = 1; i < exports.responsiveGridConfigs.length; i++) {
            const prev = exports.responsiveGridConfigs[i - 1];
            const curr = exports.responsiveGridConfigs[i];
            if (curr.columns <= prev.columns) {
                errors.push(`Column count should increase progressively: ${prev.breakpoint} (${prev.columns}) -> ${curr.breakpoint} (${curr.columns})`);
            }
        }
        return {
...
exports.ResponsiveGridGenerator = ResponsiveGridGenerator;
//# sourceMappingURL=ResponsiveGridGenerator.js.map