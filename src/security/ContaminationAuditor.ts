...
import { ContaminationPrevention, ContaminationVector } from './ContaminationPrevention';
...
export class ContaminationAuditor {
  ...
  private documentationGuard: DocumentationGuard;
  private config: ContaminationAuditorConfig;
  ...
  constructor(config: Partial<ContaminationAuditorConfig> = {}) {
    ...
    this.documentationGuard = new DocumentationGuard({
  ...
  auditTarget(target: AuditTarget): AuditResult {
    let vectors: ContaminationVector[] = [];
    let passed = true;
    ...
    if (shouldAudit) {
      const checkResult = this.contaminationPrevention.checkContent(
        target.content,
        target.type
      );
      ...
      if (target.type === 'documentation') {
        const docValidation = this.documentationGuard.validateDocumentation(
          target.content,
          target.path
        );
   ...
   */
  private shouldAuditTarget(type: AuditTarget['type']): boolean {
    switch (type) {
      case 'documentation':
   ...
   */
  private determineSeverity(vectors: ContaminationVector[]): AuditResult['severity'] {
    if (vectors.length === 0) {
      return 'clean';
    }
   ...
   */
  private generateRecommendations(target: AuditTarget, vectors: ContaminationVector[]): string[] {
    ...
    const vectorsByType = new Map<string, ContaminationVector[]>();
    ...
    for (const [type, typeVectors] of vectorsByType) {
      ...
      const uniqueSuggestions = new Set(
        typeVectors.map(v => v.suggestion).filter(Boolean)
      );
  ...
  private generateSummary(results: AuditResult[]): AuditSummary {
    ...
    // Count by severity
    const vectorsBySeverity: Record<string, number> = {};
    for (const vector of allVectors) {
