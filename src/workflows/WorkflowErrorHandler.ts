...
export class WorkflowErrorHandler {
   ...
   */
  private isCriticalError(errorMessage: string, stage: WorkflowStage): boolean {
   ...
   */
  private isRecoverableError(errorMessage: string, stage: WorkflowStage): boolean {
  ...
  private determineRecoveryStrategy(error: WorkflowError): RecoveryStrategy {
    switch (error.stage) {
      case WorkflowStage.TOKEN_REGISTRATION:
      ...
      case WorkflowStage.CONSISTENCY_CHECK:
        return this.getConsistencyRecoveryStrategy(error);
      
      case WorkflowStage.TRANSLATION:
   ...
   */
  private getRegistrationRecoveryStrategy(error: WorkflowError): RecoveryStrategy {
   ...
   */
  private getValidationRecoveryStrategy(error: WorkflowError): RecoveryStrategy {
    return {
      canRecover: true,
   ...
   */
  private getConsistencyRecoveryStrategy(error: WorkflowError): RecoveryStrategy {
    return {
      canRecover: true,
   ...
   */
  private getTranslationRecoveryStrategy(error: WorkflowError): RecoveryStrategy {
    return {
      canRecover: true,
  ...
  generateErrorReport(): {
    ...
    const errorsByStage: Record<WorkflowStage, number> = {} as any;
