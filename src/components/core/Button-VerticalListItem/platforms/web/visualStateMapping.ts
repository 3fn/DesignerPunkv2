/**
 * Visual State Mapping for Button-VerticalListItem Web Component
 * 
 * Stemma System: Buttons Family
 * Component Type: Primitive (VerticalListItem)
 * 
 * Provides visual state to CSS styling mappings for the Button-VerticalListItem
 ...
 * 
 * @module Button-VerticalListItem/platforms/web/visualStateMapping
...
export function getVisualStateStyles(state: VisualState): VisualStateStyles {
  ...
  if (!styles) {
    ...
    throw new Error(
      `Button-VerticalListItem: Invalid visual state "${state}". ` +
