 ...
 * 
 * @module Badge-Count-Notification/__tests__/test-utils
 */
...
/**
 * Set up required CSS custom properties for Badge-Count-Notification token-based styling.
 * 
 * Badge-Count-Notification uses CSS custom properties for all styling (zero hard-coded values).
...
/**
 * Register the Badge-Count-Notification custom element if not already registered.
 * Prevents "already defined" errors in test suites.
 * 
 * Also sets up required CSS custom properties for token-based styling.
 */
export function registerBadgeCountNotification(): void {
  // Set up CSS custom properties required for token-based styling
  setupBadgeCountNotificationTokens();
  
  // BadgeCountNotification auto-registers on import, but we ensure it's registered
...
/**
 * Create a Badge-Count-Notification element with shadow DOM ready.
 * 
 * @param props - Initial properties for the badge
 * @returns Promise resolving to the created badge element
 */
export async function createBadgeCountNotification(props: {
  count: number;
  max?: number;
  showZero?: boolean;
  size?: BadgeCountSize;
  ...
  testID?: string;
}): Promise<BadgeCountNotification> {
  ...
  if (props.announceChanges !== undefined) badge.announceChanges = props.announceChanges;
...
/**
 * Clean up a Badge-Count-Notification element from the DOM.
 * 
 * @param badge - The badge element to clean up
 */
export function cleanupBadgeCountNotification(badge: BadgeCountNotification): void {
  if (badge.parentNode) {
 ...
 * 
 * @param badge - The Badge-Count-Notification custom element
 * @returns The badge container element inside shadow DOM, or null if not found
 */
export function getShadowBadge(badge: BadgeCountNotification): HTMLElement | null {
  return badge.shadowRoot?.querySelector('.badge-count-notification') || null;
}

/**
 * Get the text element from a Badge-Count-Notification.
 * 
 * @param badge - The Badge-Count-Notification custom element
 * @returns The text element inside shadow DOM, or null if not found
 */
export function getTextElement(badge: BadgeCountNotification): HTMLElement | null {
  return badge.shadowRoot?.querySelector('.badge-count-notification__text') || null;
}

/**
 * Get the announcement element from a Badge-Count-Notification.
 * 
 * @param badge - The Badge-Count-Notification custom element
 * @returns The announcement element inside shadow DOM, or null if not found
 */
export function getAnnouncementElement(badge: BadgeCountNotification): HTMLElement | null {
  return badge.shadowRoot?.querySelector('.badge-count-notification__announcement') || null;
}
 ...
 */
export function hasClass(badge: BadgeCountNotification, className: string): boolean {
 ...
 */
export function isRendered(badge: BadgeCountNotification): boolean {
 ...
 */
export function getBadgeComputedStyle(badge: BadgeCountNotification): CSSStyleDeclaration | null {
  const shadowBadge = getShadowBadge(badge);
  if (shadowBadge) {
