...
export class BlendCalculator {
  ...
  calculateBlend(
    baseColor: string,
    blendToken: PrimitiveToken,
    direction: BlendDirection
  ): string {
    // Parse hex color to RGB
    const rgb: RGB = hexToRgb(baseColor);
    ...
    switch (direction) {
      case BlendDirection.DARKER:
        blendedRgb = calculateDarkerBlend(rgb, blendValue);
        break;
        
      case BlendDirection.LIGHTER:
        blendedRgb = calculateLighterBlend(rgb, blendValue);
        break;
        
      case BlendDirection.SATURATE:
        blendedRgb = calculateSaturateBlend(rgb, blendValue);
        break;
        
      case BlendDirection.DESATURATE:
        blendedRgb = calculateDesaturateBlend(rgb, blendValue);
        break;
