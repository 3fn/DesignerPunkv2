...
import { ValidationResult, ValidationError, ValidationWarning, ReleasePlan, VersionBump } from '../types/ReleaseTypes';
import { ReleaseConfig } from '../config/ReleaseConfig';
import { SemanticVersionValidator } from './SemanticVersionValidator';
import { ReleaseReadinessValidator } from './ReleaseReadinessValidator';
import { SafetyValidator } from './SafetyValidator';

export class ReleaseValidator {
  private config: ReleaseConfig;
  private semanticVersionValidator: SemanticVersionValidator;
  private releaseReadinessValidator: ReleaseReadinessValidator;
  private safetyValidator: SafetyValidator;

  constructor(config: ReleaseConfig) {
    this.config = config;
    this.semanticVersionValidator = new SemanticVersionValidator(config.versioning.semanticVersioning);
    this.releaseReadinessValidator = new ReleaseReadinessValidator(config.validation.validationRules);
    this.safetyValidator = new SafetyValidator(config.validation.safetyChecks);
  }

  /**
   * Comprehensive release validation
   */
  async validateRelease(releasePlan: ReleasePlan): Promise<ValidationResult> {
   ...
   */
  async validateReleaseReadiness(releaseData: any): Promise<ValidationResult> {
   ...
   */
  async validateSemanticVersioning(version: string, bumpType: 'major' | 'minor' | 'patch'): Promise<ValidationResult> {
    const errors: ValidationError[] = [];
    const warnings: ValidationWarning[] = [];
   ...
   * Validate package compatibility
   */
  async validatePackageCompatibility(packages: any[]): Promise<ValidationResult> {
    const errors: ValidationError[] = [];
    const warnings: ValidationWarning[] = [];
   ...
   * Validate safety checks
   */
  async validateSafetyChecks(releaseData: any): Promise<ValidationResult> {
    const errors: ValidationError[] = [];
    const warnings: ValidationWarning[] = [];
   ...
   */
  private validateDocumentSections(doc: any): string | null {
   ...
   * Check for dependency conflicts
   */
  private checkDependencyConflict(dependency: any, packages: any[]): string | null {
   ...
   */
  async validateRollbackCapability(releasePlan: ReleasePlan): Promise<ValidationResult> {
   ...
   */
  async validateReleasePlan(releasePlan: ReleasePlan): Promise<ValidationResult> {
    const errors: ValidationError[] = [];
    const warnings: ValidationWarning[] = [];
