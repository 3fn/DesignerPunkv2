...
import { DEFAULT_RELEASE_CONFIG } from '../../config/ReleaseConfig';
import { ReleasePlan, VersionBump } from '../../types/ReleaseTypes';

describe('Validation Framework', () => {
  let releaseValidator: ReleaseValidator;
  let semanticVersionValidator: SemanticVersionValidator;
  let releaseReadinessValidator: ReleaseReadinessValidator;
  let safetyValidator: SafetyValidator;

  beforeEach(() => {
    releaseValidator = new ReleaseValidator(DEFAULT_RELEASE_CONFIG);
    semanticVersionValidator = new SemanticVersionValidator(DEFAULT_RELEASE_CONFIG.versioning.semanticVersioning);
    releaseReadinessValidator = new ReleaseReadinessValidator(DEFAULT_RELEASE_CONFIG.validation.validationRules);
    safetyValidator = new SafetyValidator(DEFAULT_RELEASE_CONFIG.validation.safetyChecks);
  });
  ...
  describe('ReleaseValidator', () => {
    ...
    it('should validate release plan structure', async () => {
      const mockReleasePlan: Partial<ReleasePlan> = {
        id: 'test-release-1',
        version: {
          from: '1.0.0',
          to: '1.1.0',
      ...
      const result = await releaseValidator.validateReleasePlan(mockReleasePlan as ReleasePlan);
  ...
  describe('ReleaseReadinessValidator', () => {
    it('should validate release plan structure', async () => {
      ...
      const result = await releaseReadinessValidator.validateReleaseReadiness(mockReleasePlan as ReleasePlan);
  ...
  describe('SafetyValidator', () => {
    it('should validate release safety', async () => {
      ...
      const result = await safetyValidator.validateReleaseSafety(mockReleasePlan as ReleasePlan);
  ...
  describe('Integration Tests', () => {
    it('should perform comprehensive validation', async () => {
      const mockReleasePlan: ReleasePlan = {
