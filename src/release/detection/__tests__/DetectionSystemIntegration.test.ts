...
import { ReleaseSignal, ReleaseAnalysis } from '../../types/ReleaseTypes';
...
`;
      ...
      mockFs.readdir.mockResolvedValue(['spec-completion-summary.md'] as any);
      ...
      expect(analysis.breakingChanges.some(bc => 
        bc.title.includes('validateOld') || bc.description.includes('validateOld')
      )).toBe(true);

      // Verify breaking changes include configuration format change
      expect(analysis.breakingChanges.some(bc => 
        bc.title.includes('configuration file format') || bc.description.includes('configuration file format')
      )).toBe(true);
    });
