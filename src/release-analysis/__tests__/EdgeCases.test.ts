...
describe('Release Analysis Edge Cases', () => {
  ...
  beforeEach(() => {
    ...
    generator = new ReleaseNoteGenerator();
  });
...
`,
    ...
    it('should handle extremely long documents', async () => {
      const longContent = Array.from({ length: 1000 }, (_, i) => 
        `- Change item ${i}: This is a very long description that goes on and on with lots of details about what changed in this particular item number ${i}.`
      ).join('\n');
  ...
  describe('Version Calculation Edge Cases', () => {
    ...
    it('should handle conflicting change types', () => {
      const conflictingChanges: ExtractedChanges = {
        breakingChanges: [{
          id: '1',
          title: 'Breaking change',
          description: 'Major change',
  ...
  describe('Release Note Generation Edge Cases', () => {
    it('should handle changes with missing or null fields', async () => {
      const changesWithNulls: ExtractedChanges = {
        breakingChanges: [{
          id: '1',
          title: 'Breaking change',
    ...
    it('should handle extremely long change descriptions', async () => {
      ...
      const changesWithLongDescriptions: ExtractedChanges = {
        breakingChanges: [{
          id: '1',
    ...
    it('should handle special characters in change titles and descriptions', async () => {
      const changesWithSpecialChars: ExtractedChanges = {
        breakingChanges: [{
          id: '1',
  ...
  describe('Deduplication Edge Cases', () => {
    it('should handle near-duplicate changes with slight variations', async () => {
      const changesWithDuplicates: ExtractedChanges = {
        breakingChanges: [],
        newFeatures: [
          {
            id: '1',
    ...
    it('should preserve distinct changes that appear similar', async () => {
      const changesWithSimilarButDistinct: ExtractedChanges = {
        breakingChanges: [],
        newFeatures: [
          {
            id: '1',
  ...
  describe('Confidence Calculation Edge Cases', () => {
    it('should handle confidence calculation with extreme values', () => {
      const extremeChanges: ExtractedChanges = {
        breakingChanges: [],
        newFeatures: [],
        bugFixes: [],
        improvements: [],
    ...
    it('should handle confidence calculation with perfect scores', () => {
      const perfectChanges: ExtractedChanges = {
        breakingChanges: [],
        newFeatures: [{
          id: '1',
