...
import { ExtractionConfig, DEFAULT_ANALYSIS_CONFIG } from '../../config/AnalysisConfig';
...
describe('SimpleChangeExtractor - Confidence Metrics Integration', () => {
  ...
  beforeEach(() => {
    config = DEFAULT_ANALYSIS_CONFIG.extraction;
    extractor = new SimpleChangeExtractor(config);
  });
...
`,
  ...
  describe('confidence threshold handling', () => {
    it('should respect confidence thresholds in validation', async () => {
      ...
      const strictExtractor = new SimpleChangeExtractor(strictConfig);
    ...
    it('should pass validation with lenient thresholds', async () => {
      ...
      const lenientExtractor = new SimpleChangeExtractor(lenientConfig);
