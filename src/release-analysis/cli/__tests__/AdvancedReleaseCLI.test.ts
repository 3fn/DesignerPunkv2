...
import { AnalysisResult, AnalysisOptions, InteractiveOptions } from '../../types/AnalysisTypes';
...
describe('AdvancedReleaseCLI', () => {
  ...
  let mockMkdtemp: jest.MockedFunction<typeof fs.mkdtemp>;

  beforeEach(async () => {
    mockMkdtemp = fs.mkdtemp as jest.MockedFunction<typeof fs.mkdtemp>;
    mockMkdtemp.mockResolvedValue('/tmp/test-dir');
    
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'release-cli-test-'));
    cli = new AdvancedReleaseCLI(tempDir);
  ...
  describe('Dry-run and Preview', () => {
    it('should show analysis preview in dry-run mode', async () => {
        ...
        completionDocuments: [
          {
            ...
            metadata: { type: 'task-completion' as const }
          }
        ],
      ...
      jest.spyOn(cli as any, 'previewAnalysis').mockResolvedValue(mockScope);
      jest.spyOn(cli as any, 'promptUser').mockResolvedValue('n');
  ...
  describe('Interactive Mode', () => {
    it('should handle interactive review of ambiguous items', async () => {
      ...
      const mockPromptUser = jest.spyOn(cli as any, 'promptUser')
      ...
      await (cli as any).interactiveReview(mockChanges, { interactive: true });
    ...
    it('should auto-approve items when autoApprove is enabled', async () => {
      ...
      await (cli as any).interactiveReview(mockChanges, { 
        interactive: true, 
        autoApprove: true 
      });
  ...
  describe('Argument Parsing', () => {
    it('should parse basic analysis arguments', () => {
      ...
      const result = (cli as any).parseAdvancedArguments(args);
  ...
  describe('Version Override', () => {
    it('should allow version override from major to minor', async () => {
      ...
      const override = await (cli as any).promptVersionOverride(currentRecommendation);
  ...
  describe('Error Handling', () => {
    it('should handle configuration loading errors gracefully', async () => {
      ...
      await (cli as any).loadConfiguration();
      
      expect((cli as any).config).toBeDefined();
      expect((cli as any).config.extraction).toBeDefined();
    });
  ...
  describe('Report Display', () => {
    ...
    beforeEach(() => {
      mockResult = {
        ...
        changes: {
          breakingChanges: [{ id: '1', title: 'Breaking change', description: 'Test', affectedAPIs: [], source: 'test', severity: 'high' as const }],
