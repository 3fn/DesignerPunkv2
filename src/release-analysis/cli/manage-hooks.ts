...
import { HookIntegrationManager, HookConfig } from '../hooks/HookIntegrationManager';
...
async function manageHooks(options: ManageHooksOptions): Promise<void> {
  try {
    // Load configuration
    ...
    // Create hook manager
    const hookManager = new HookIntegrationManager(config, hookConfig);
...
async function installHooks(
  manager: HookIntegrationManager,
  config: HookConfig
): Promise<void> {
  console.log('📦 Installing release analysis hooks...\n');

  const hookTypes: Array<'git' | 'agent'> = 
    config.hookType === 'both' ? ['git', 'agent'] : [config.hookType];
...
async function validateHooks(manager: HookIntegrationManager): Promise<void> {
...
async function showStatus(manager: HookIntegrationManager): Promise<void> {
...
function parseArgs(): ManageHooksOptions {
  ...
  const action = args[0] as ManageHooksOptions['action'];
  const options: ManageHooksOptions = { action };
  ...
  for (let i = 1; i < args.length; i++) {
    ...
    switch (arg) {
      case '--type':
        options.hookType = args[++i] as 'git' | 'agent' | 'both';
        break;
      case '--quick':
