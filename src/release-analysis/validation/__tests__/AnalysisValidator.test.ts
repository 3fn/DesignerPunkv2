...
import { AnalysisValidator, ValidationConfig, AnalysisValidationResult } from '../AnalysisValidator';
...
describe('AnalysisValidator', () => {
  ...
  let mockChanges: ExtractedChanges;
  let mockVersionRecommendation: VersionRecommendation;
...
- **Feature B**: Improves user experience with better interface`;
    ...
    it('should fail validation for critical quality gate failures', () => {
      ...
      expect(result.details.qualityGates.some(gate => 
        gate.name === 'extraction_confidence' && !gate.passed && gate.critical
      )).toBe(true);
    });
...
- Various improvements and other changes`;
      ...
      expect(result.warnings).toContainEqual(
        expect.stringContaining('ambiguous items detected')
      );
    });
  });
  ...
  describe('validateVersionRecommendation', () => {
    ...
    it('should detect invalid semantic version format', () => {
      ...
      expect(result.errors).toContainEqual(
        expect.stringContaining('Current version "invalid-version" is not a valid semantic version')
      );
    });
    ...
    it('should detect invalid version progression', () => {
      ...
      expect(result.errors).toContainEqual(
        expect.stringContaining('Version progression from 1.0.0 to 2.0.0 is invalid for minor bump')
      );
    });
    ...
    it('should detect incorrect bump type for breaking changes', () => {
      ...
      expect(result.errors).toContainEqual(
        expect.stringContaining('Recommended minor bump is insufficient for detected changes (expected major)')
      );
    });
    ...
    it('should warn about low confidence for major version bumps', () => {
      ...
      expect(result.warnings).toContainEqual(
        expect.stringContaining('Low confidence (0.60) for major version bump - manual review recommended')
      );
    });
...
- New feature added`;
      ...
      expect(result.errors).toContain(
        'Breaking changes detected but not documented in release notes'
      );
    });
...
- Change`;
      ...
      expect(result.suggestions).toContainEqual(
        expect.stringContaining('change descriptions are very short - consider adding more detail')
      );
    });
...
- Some bug fixes and other changes`;
      ...
      expect(result.suggestions).toContain(
        'Consider replacing vague terms with specific descriptions'
      );
    });
  });
  ...
  describe('validateConfidence', () => {
    ...
    it('should detect confidence below minimum threshold', () => {
      ...
      expect(result.errors).toContainEqual(
        expect.stringContaining('Overall confidence (0.40) below minimum threshold')
      );
      expect(result.errors).toContainEqual(
        expect.stringContaining('Extraction confidence (0.40) below threshold')
      );
    });
    ...
    it('should warn about ambiguous items affecting confidence', () => {
      ...
      expect(result.warnings).toContain(
        '2 ambiguous items detected - may affect confidence'
      );
    });
  });
...
- Well documented feature with comprehensive description`;
    ...
    it('should fail critical quality gates', () => {
      ...
      const extractionGate = result.details.qualityGates.find(gate => 
        gate.name === 'extraction_confidence'
      );
  ...
  describe('custom configuration', () => {
    it('should use custom confidence thresholds', () => {
      ...
      const customValidator = new AnalysisValidator(customConfig);
      ...
      expect(result.valid).toBe(false);
      expect(result.errors).toContainEqual(
        expect.stringContaining('below minimum threshold (0.9)')
      );
    });
...
- Short description`; // Below 50 character minimum
      ...
      expect(result.suggestions).toContainEqual(
        expect.stringContaining('change descriptions are very short')
      );
    });
  });
});