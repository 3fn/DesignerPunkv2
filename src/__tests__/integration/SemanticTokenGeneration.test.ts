...
describe('Semantic Token Generation - End-to-End Integration', () => {
  ...
  describe('Backward Compatibility', () => {
    it('should maintain primitive token output unchanged', () => {
      ...
      // Motion primitives (easing, duration, scale) are generated by the dedicated
      // Motion Tokens section with platform-native naming, not the primitive pass
      const MOTION_CATEGORIES = new Set(['easing', 'duration', 'scale']);
      const nonMotionPrimitives = primitives.filter(t => !MOTION_CATEGORIES.has(t.category));
