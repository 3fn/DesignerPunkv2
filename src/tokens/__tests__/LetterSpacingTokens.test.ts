...
describe('Letter Spacing Tokens', () => {
  describe('Token Structure and Organization', () => {
    ...
    test('should have correct token category for all letter spacing tokens', () => {
      const allTokens = getAllLetterSpacingTokens();
      
      expect(allTokens.length).toBeGreaterThan(0);
      expect(allTokens.every(token => token.category === TokenCategory.LETTER_SPACING)).toBe(true);
      expect(allTokens.every(token => token.familyBaseValue === LETTER_SPACING_BASE_VALUE)).toBe(true);
    });

    test('should have consistent token naming pattern', () => {
      const tokenNames = letterSpacingTokenNames;
  ...
  describe('Em-Based Value Progression', () => {
    test('should have 0 as base letter spacing', () => {
      const letterSpacing100 = getLetterSpacingToken('letterSpacing100');
      
      expect(letterSpacing100).toBeDefined();
      expect(letterSpacing100?.baseValue).toBe(LETTER_SPACING_BASE_VALUE);
      expect(letterSpacing100?.description).toContain('base unit');
    });

    test('should have correct em-based values for each token', () => {
      expect(getLetterSpacingToken('letterSpacing025')?.baseValue).toBe(-0.025);
      expect(getLetterSpacingToken('letterSpacing050')?.baseValue).toBe(-0.05);
      expect(getLetterSpacingToken('letterSpacing100')?.baseValue).toBe(0);
      expect(getLetterSpacingToken('letterSpacing125')?.baseValue).toBe(0.025);
      expect(getLetterSpacingToken('letterSpacing150')?.baseValue).toBe(0.05);
    });

    test('should provide both tight and loose spacing options', () => {
      const allTokens = getAllLetterSpacingTokens();
      const baseValue = LETTER_SPACING_BASE_VALUE;
    ...
    test('should have appropriate semantic descriptions', () => {
      expect(getLetterSpacingToken('letterSpacing025')?.description).toContain('Tight letter spacing');
      expect(getLetterSpacingToken('letterSpacing050')?.description).toContain('Very tight letter spacing');
      expect(getLetterSpacingToken('letterSpacing100')?.description).toContain('Normal letter spacing');
      expect(getLetterSpacingToken('letterSpacing125')?.description).toContain('Loose letter spacing');
      expect(getLetterSpacingToken('letterSpacing150')?.description).toContain('Very loose letter spacing');
    });

    test('should reference appropriate use cases in descriptions', () => {
      expect(getLetterSpacingToken('letterSpacing025')?.description).toContain('large text');
      expect(getLetterSpacingToken('letterSpacing050')?.description).toContain('display text');
      expect(getLetterSpacingToken('letterSpacing125')?.description).toContain('small text');
      expect(getLetterSpacingToken('letterSpacing150')?.description).toContain('emphasis');
    });
  });

  describe('Mathematical Relationships', () => {
    test('should have correct mathematical relationships from base 0', () => {
      const letterSpacing025 = getLetterSpacingToken('letterSpacing025');
      const letterSpacing050 = getLetterSpacingToken('letterSpacing050');
      const letterSpacing100 = getLetterSpacingToken('letterSpacing100');
      const letterSpacing125 = getLetterSpacingToken('letterSpacing125');
      const letterSpacing150 = getLetterSpacingToken('letterSpacing150');
    ...
    test('should maintain symmetric progression around base value', () => {
      const base = LETTER_SPACING_BASE_VALUE;
      
      expect(getLetterSpacingToken('letterSpacing025')?.baseValue).toBe(base - 0.025);
      expect(getLetterSpacingToken('letterSpacing050')?.baseValue).toBe(base - 0.05);
      expect(getLetterSpacingToken('letterSpacing100')?.baseValue).toBe(base);
      expect(getLetterSpacingToken('letterSpacing125')?.baseValue).toBe(base + 0.025);
      expect(getLetterSpacingToken('letterSpacing150')?.baseValue).toBe(base + 0.05);
    });

    test('should use consistent increment values', () => {
      const increment = 0.025;
      
      expect(getLetterSpacingToken('letterSpacing025')?.baseValue).toBe(-increment);
      expect(getLetterSpacingToken('letterSpacing050')?.baseValue).toBe(-increment * 2);
      expect(getLetterSpacingToken('letterSpacing125')?.baseValue).toBe(increment);
      expect(getLetterSpacingToken('letterSpacing150')?.baseValue).toBe(increment * 2);
    });
  });
  ...
  describe('Precision Targeting for Typography', () => {
    ...
    test('should support both tightening and loosening', () => {
      const negativeValues = getAllLetterSpacingTokens().filter(token => token.baseValue < 0);
      const positiveValues = getAllLetterSpacingTokens().filter(token => token.baseValue > 0);
  ...
  describe('Token Retrieval Functions', () => {
    ...
    test('should return undefined for non-existent tokens', () => {
      const nonExistent = getLetterSpacingToken('letterSpacing075');
      const invalid = getLetterSpacingToken('letterSpacing200');
      const wrongFormat = getLetterSpacingToken('spacing100');
    ...
    test('should have consistent token names array', () => {
      const tokenNames = letterSpacingTokenNames;
      const tokenKeys = Object.keys(letterSpacingTokens);
  ...
  describe('Typography Use Cases', () => {
    test('should support display typography with tight spacing', () => {
      const tightSpacing = getLetterSpacingToken('letterSpacing025');
      const veryTightSpacing = getLetterSpacingToken('letterSpacing050');
    ...
    test('should support small text with loose spacing', () => {
      const looseSpacing = getLetterSpacingToken('letterSpacing125');
      const veryLooseSpacing = getLetterSpacingToken('letterSpacing150');
    ...
    test('should provide normal spacing as baseline', () => {
      const normalSpacing = getLetterSpacingToken('letterSpacing100');
  ...
  describe('Integration with Token System', () => {
    ...
    test('should maintain mathematical consistency with family base value', () => {
      ...
      allTokens.forEach(token => {
        expect(token.familyBaseValue).toBe(LETTER_SPACING_BASE_VALUE);
