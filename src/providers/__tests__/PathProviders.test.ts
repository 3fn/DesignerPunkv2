...
import { WebFileOrganizer } from '../WebFileOrganizer';
import { iOSFileOrganizer } from '../iOSFileOrganizer';
import { AndroidFileOrganizer } from '../AndroidFileOrganizer';
import { OutputFormat } from '../../types/TranslationOutput';

describe('WebFileOrganizer', () => {
  let organizer: WebFileOrganizer;

  beforeEach(() => {
    organizer = new WebFileOrganizer();
  });
  ...
  describe('File Naming', () => {
    ...
    it('should throw error for unsupported format', () => {
      expect(() => organizer.getFileName('swift' as OutputFormat)).toThrow(
        'Unsupported format for web platform: swift'
      );
    });
  });
...
describe('iOSFileOrganizer', () => {
  let organizer: iOSFileOrganizer;

  beforeEach(() => {
    organizer = new iOSFileOrganizer();
  });
  ...
  describe('File Naming', () => {
    ...
    it('should throw error for unsupported format', () => {
      expect(() => organizer.getFileName('javascript' as OutputFormat)).toThrow(
        'Unsupported format for iOS platform: javascript'
      );
    });
  });
...
describe('AndroidFileOrganizer', () => {
  let organizer: AndroidFileOrganizer;

  beforeEach(() => {
    organizer = new AndroidFileOrganizer();
  });
  ...
  describe('File Naming', () => {
    ...
    it('should throw error for unsupported format', () => {
      expect(() => organizer.getFileName('swift' as OutputFormat)).toThrow(
        'Unsupported format for Android platform: swift'
      );
    });
  });
  ...
  describe('Build System Optimization', () => {
    it('should optimize for Kotlin and XML resources', () => {
      ...
      expect(optimized.primaryFile).toBe(
        'designsystem/src/main/kotlin/com/designsystem/tokens/DesignTokens.kt'
      );
      expect(optimized.supportingFiles).toContain(
        'designsystem/src/main/res/values/design_tokens.xml'
      );
  ...
  describe('Path Validation', () => {
    it('should validate correct Kotlin token file paths', () => {
      const result = organizer.validatePath(
        'designsystem/src/main/kotlin/com/designsystem/tokens/DesignTokens.kt'
      );
      expect(result.valid).toBe(true);
      expect(result.errors).toBeUndefined();
    ...
    it('should reject paths without designsystem/src/main directory', () => {
      ...
      expect(result.errors).toContain(
        'Android token files should be in designsystem/src/main directory'
      );
    });
    ...
    it('should reject Kotlin paths without correct package structure', () => {
      ...
      expect(result.valid).toBe(false);
      expect(result.errors).toContain(
        'Kotlin token files should be in kotlin/com/designsystem/tokens package'
      );
    });
  ...
  describe('Resource Qualifiers', () => {
    it('should provide resource qualifier paths for mode-aware resources', () => {
      ...
      expect(qualifiers.densities.xhdpi).toBe(
        'designsystem/src/main/res/values-xhdpi/design_tokens.xml'
      );
    });
...
describe('Cross-Platform Path Consistency', () => {
  let webOrganizer: WebFileOrganizer;
  let iosOrganizer: iOSFileOrganizer;
  let androidOrganizer: AndroidFileOrganizer;

  beforeEach(() => {
    webOrganizer = new WebFileOrganizer();
    iosOrganizer = new iOSFileOrganizer();
    androidOrganizer = new AndroidFileOrganizer();
  });
