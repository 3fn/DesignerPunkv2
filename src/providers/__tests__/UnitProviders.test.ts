...
import { WebUnitConverter } from '../WebUnitConverter';
import { iOSUnitConverter } from '../iOSUnitConverter';
import { AndroidUnitConverter, AndroidDensity } from '../AndroidUnitConverter';
import { UnitProvider, UnitConversionConfig } from '../UnitProvider';
...
describe('Unit Provider Services', () => {
  describe('WebUnitConverter', () => {
    let webConverter: WebUnitConverter;

    beforeEach(() => {
      webConverter = new WebUnitConverter();
    });
    ...
    describe('Custom Configuration', () => {
      test('should use custom base font size for REM calculations', () => {
        const customConverter = new WebUnitConverter({ baseFontSize: 14 });
  ...
  describe('iOSUnitConverter', () => {
    let iosConverter: iOSUnitConverter;

    beforeEach(() => {
      iosConverter = new iOSUnitConverter();
    });
    ...
    describe('Display Scale Handling', () => {
      ...
      test('should support custom display scales', () => {
        const retina2xConverter = new iOSUnitConverter({ displayScale: 2.0 });
        const retina3xConverter = new iOSUnitConverter({ displayScale: 3.0 });
  ...
  describe('AndroidUnitConverter', () => {
    let androidConverter: AndroidUnitConverter;

    beforeEach(() => {
      androidConverter = new AndroidUnitConverter();
    });
    ...
    describe('Android Density Buckets', () => {
      test('should use MDPI as default density', () => {
        expect(androidConverter.getDensityFactor()).toBe(AndroidDensity.MDPI);
      });

      test('should support all density buckets', () => {
        expect(AndroidDensity.LDPI).toBe(0.75);
        expect(AndroidDensity.MDPI).toBe(1.0);
        expect(AndroidDensity.HDPI).toBe(1.5);
        expect(AndroidDensity.XHDPI).toBe(2.0);
        expect(AndroidDensity.XXHDPI).toBe(3.0);
        expect(AndroidDensity.XXXHDPI).toBe(4.0);
      });

      test('should convert dp to pixels correctly for different densities', () => {
        expect(androidConverter.dpToPixels(48, AndroidDensity.LDPI)).toBe(36); // 48 × 0.75
        expect(androidConverter.dpToPixels(48, AndroidDensity.MDPI)).toBe(48); // 48 × 1.0
        expect(androidConverter.dpToPixels(48, AndroidDensity.HDPI)).toBe(72); // 48 × 1.5
        expect(androidConverter.dpToPixels(48, AndroidDensity.XHDPI)).toBe(96); // 48 × 2.0
        expect(androidConverter.dpToPixels(48, AndroidDensity.XXHDPI)).toBe(144); // 48 × 3.0
        expect(androidConverter.dpToPixels(48, AndroidDensity.XXXHDPI)).toBe(192); // 48 × 4.0
      });

      test('should convert pixels to dp correctly for different densities', () => {
        expect(androidConverter.pixelsToDp(48, AndroidDensity.MDPI)).toBe(48); // 48 ÷ 1.0
        expect(androidConverter.pixelsToDp(96, AndroidDensity.XHDPI)).toBe(48); // 96 ÷ 2.0
        expect(androidConverter.pixelsToDp(144, AndroidDensity.XXHDPI)).toBe(48); // 144 ÷ 3.0
      });

      test('should convert sp to pixels with font scale consideration', () => {
        expect(androidConverter.spToPixels(16, AndroidDensity.MDPI, 1.0)).toBe(16); // Normal scale
        expect(androidConverter.spToPixels(16, AndroidDensity.MDPI, 1.5)).toBe(24); // Large text
        expect(androidConverter.spToPixels(16, AndroidDensity.XHDPI, 1.0)).toBe(32); // High density
        expect(androidConverter.spToPixels(16, AndroidDensity.XHDPI, 1.5)).toBe(48); // High density + large text
      });
    });
  ...
  describe('Cross-Platform Mathematical Consistency', () => {
    ...
    describe('Proportional Relationships', () => {
      test('should maintain proportional relationships for spacing tokens', () => {
        const spacing8 = createMockToken({ category: TokenCategory.SPACING, baseValue: 8 });
        const spacing16 = createMockToken({ category: TokenCategory.SPACING, baseValue: 16 });
        ...
        expect((web16.value as number) / (web8.value as number)).toBe(2);
        expect((ios16.value as number) / (ios8.value as number)).toBe(2);
        expect((android16.value as number) / (android8.value as number)).toBe(2);
      });

      test('should maintain proportional relationships for fontSize tokens', () => {
        const fontSize16 = createMockToken({ category: TokenCategory.FONT_SIZE, baseValue: 16 });
        const fontSize18 = createMockToken({ category: TokenCategory.FONT_SIZE, baseValue: 18 });
        ...
        // Calculate ratios
        const webRatio = (web18.value as number) / (web16.value as number);
        const iosRatio = (ios18.value as number) / (ios16.value as number);
        const androidRatio = (android18.value as number) / (android16.value as number);
      ...
      test('should maintain identical values for unitless tokens', () => {
        const lineHeight = createMockToken({ category: TokenCategory.LINE_HEIGHT, baseValue: 1.5 });
        const density = createMockToken({ category: TokenCategory.DENSITY, baseValue: 1.0 });
    ...
    describe('Mathematical Equivalence', () => {
      ...
      test('should handle REM conversion precision correctly', () => {
        const fontSize24 = createMockToken({ category: TokenCategory.FONT_SIZE, baseValue: 24 });
        ...
        const webPixelEquivalent = (webResult.value as number) * 16;
  ...
  describe('Error Handling and Edge Cases', () => {
    ...
    test('should validate conversion accuracy', () => {
      const providers: UnitProvider[] = [webConverter, iosConverter, androidConverter];
