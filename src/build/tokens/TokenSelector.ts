...
import { TokenSelection, TokenSelectionOptions, TokenPriority } from './TokenSelection';
import { ComponentToken, ComponentTokenSpec } from './ComponentToken';
...
export class TokenSelector {
  ...
  private trySemanticToken(
  ...
  ): { token: SemanticToken; reasoning: string } | { insufficiencyReason: string } | null {
    ...
    if (options.preferredCategory) {
      const filtered = semanticTokens.filter(t => 
        t.name.includes(options.preferredCategory!)
      );
  ...
  private tryPrimitiveToken(
    request: TokenRequest,
    options: TokenSelectionOptions
  ): { token: PrimitiveToken; reasoning: string } | { insufficiencyReason: string } | null {
    // If specific token name requested, try to get it
    if (request.tokenName) {
    ...
    // If preferred category specified in options, filter by that
    if (options.preferredCategory) {
      const filtered = primitiveTokens.filter(t => 
        t.name.includes(options.preferredCategory!)
      );
      if (filtered.length > 0) {
        const token = filtered[0];
   ...
   * Map request category to semantic category
   */
  private mapToSemanticCategory(category: string): SemanticCategory | null {
    const mapping: Record<string, SemanticCategory> = {
      'color': SemanticCategory.COLOR,
      'spacing': SemanticCategory.SPACING,
      'typography': SemanticCategory.TYPOGRAPHY,
      'border': SemanticCategory.BORDER,
      'shadow': SemanticCategory.SHADOW,
      'layout': SemanticCategory.LAYOUT,
      'interaction': SemanticCategory.INTERACTION
    };
   ...
   * Map request category to token category
   */
  private mapToTokenCategory(category: string): TokenCategory | null {
    const mapping: Record<string, TokenCategory> = {
      'spacing': TokenCategory.SPACING,
      'fontsize': TokenCategory.FONT_SIZE,
      'fontfamily': TokenCategory.FONT_FAMILY,
      'fontweight': TokenCategory.FONT_WEIGHT,
      'lineheight': TokenCategory.LINE_HEIGHT,
      'letterspacing': TokenCategory.LETTER_SPACING,
      'radius': TokenCategory.RADIUS,
      'density': TokenCategory.DENSITY,
      'taparea': TokenCategory.TAP_AREA,
