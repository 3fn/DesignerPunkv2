...
import { MethodSignatureValidator } from './MethodSignatureValidator';
...
export class InterfaceValidator {
  ...
  constructor() {
    this.methodSignatureValidator = new MethodSignatureValidator();
    this.propertyTypeValidator = new PropertyTypeValidator();
  }
  ...
  private validateSingleInterface(
    ...
    for (const other of otherInterfaces) {
      // Validate properties
      const propertyErrors = this.validateProperties(
        iface.properties,
        other.properties,
        iface.platform,
        other.platform
      ...
      // Validate methods
      const methodErrors = this.validateMethods(
        iface.methods,
        other.methods,
        iface.platform,
        other.platform
      ...
      // Validate events
      const eventErrors = this.validateEvents(
        iface.events,
        other.events,
        iface.platform,
        other.platform
      ...
      // Validate states
      const stateErrors = this.validateStates(
        iface.states,
        other.states,
        iface.platform,
        other.platform
  ...
  validateProperties(
    ...
    otherProperties: PropertyDefinition[],
    platform: Platform,
    otherPlatform: Platform
  ): ValidationError[] {
    const result = this.propertyTypeValidator.validateProperties(
      properties,
      otherProperties,
      platform,
      otherPlatform
  ...
  validateMethods(
    ...
    const result = this.methodSignatureValidator.validateMethodSignatures(
      methods,
      otherMethods,
      platform,
      otherPlatform
   ...
   */
  private summarizeErrors(errors: ValidationError[]): {
    total: number;
    ...
    return {
      total: errors.length,
      byType: byType as Record<ValidationErrorType, number>,
    };
  }
   ...
   */
  private summarizeWarnings(warnings: ValidationWarning[]): {
    total: number;
    ...
    return {
      ...
      byType: byType as Record<ValidationWarningType, number>,
    };
  }
}
